...
def test_a_ghost_story_controlled_artifact_is_reader_first_audio_hidden():
    ...
    assert status["audiobook_enabled"] is False
    assert artifact is not None
    assert artifact["slug"] == "a-ghost-story"
    assert artifact["title"] == "A Ghost Story"
    assert artifact["audio_enabled"] is False
...
def test_a_ghost_story_public_detail_falls_back_to_artifact_without_audio(monkeypatch):
...
def test_a_ghost_story_reader_manifest_has_content_and_no_audio(monkeypatch):
    monkeypatch.setattr(server, "db", SimpleNamespace(books=EmptyBooks()))
