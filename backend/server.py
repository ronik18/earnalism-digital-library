...
# Growth OS starts as a deterministic, admin-supervised control plane. Keep
# production execution off unless both flags are deliberately enabled.
GROWTH_OS_ENABLED = _env_bool("GROWTH_OS_ENABLED", True)
GROWTH_OS_DRY_RUN_ONLY = _env_bool("GROWTH_OS_DRY_RUN_ONLY", True)
GROWTH_OS_PUBLIC_EXECUTION_ENABLED = _env_bool("GROWTH_OS_PUBLIC_EXECUTION_ENABLED", False)
GROWTH_OS_DAILY_BUDGET_INR = _env_int("GROWTH_OS_DAILY_BUDGET_INR", 500, minimum=0)
GROWTH_OS_MAX_ACTIONS_PER_RUN = _env_int("GROWTH_OS_MAX_ACTIONS_PER_RUN", 12)
GROWTH_OS_MAX_MESSAGES_PER_DAY = _env_int("GROWTH_OS_MAX_MESSAGES_PER_DAY", 100)
GROWTH_OS_MAX_SOCIAL_POSTS_PER_DAY = _env_int("GROWTH_OS_MAX_SOCIAL_POSTS_PER_DAY", 2)
GROWTH_OS_MAX_LANDING_PAGES_PER_DAY = _env_int("GROWTH_OS_MAX_LANDING_PAGES_PER_DAY", 1)
GROWTH_OS_MAX_CREATOR_OUTREACH_PER_DAY = _env_int("GROWTH_OS_MAX_CREATOR_OUTREACH_PER_DAY", 10)
GROWTH_OS_MAX_INSTITUTION_OUTREACH_PER_DAY = _env_int("GROWTH_OS_MAX_INSTITUTION_OUTREACH_PER_DAY", 10)
...
class GrowthRunIn(BaseModel):
    mode: str = "dry_run"
    objective: str = "daily_growth_loop"
    selected_agents: List[str] = Field(default_factory=list)
    max_actions: int = GROWTH_OS_MAX_ACTIONS_PER_RUN


class GrowthKillSwitchIn(BaseModel):
    key: str
    enabled: bool = False
    reason: str = ""
...
GROWTH_TOOL_DEFINITIONS = {
    "create_campaign": {"public": False, "writes": True},
    "create_campaign_variant": {"public": False, "writes": True},
    "schedule_email": {"public": True, "writes": True},
    "schedule_whatsapp": {"public": True, "writes": True},
    "create_landing_page_draft": {"public": False, "writes": True},
    "publish_landing_page": {"public": True, "writes": True},
    "create_social_post_draft": {"public": False, "writes": True},
    "schedule_social_post": {"public": True, "writes": True},
    "create_referral_code": {"public": False, "writes": True},
    "issue_coupon": {"public": False, "writes": True},
    "create_institution_lead": {"public": False, "writes": True},
    "send_institution_outreach": {"public": True, "writes": True},
    "create_creator_affiliate_code": {"public": False, "writes": True},
    "update_crm_stage": {"public": False, "writes": True},
    "recommend_books": {"public": False, "writes": False},
    "start_reading_challenge": {"public": False, "writes": True},
    "send_reading_reminder": {"public": True, "writes": True},
    "answer_support_ticket": {"public": True, "writes": True},
    "create_support_ticket": {"public": False, "writes": True},
    "query_metrics": {"public": False, "writes": False},
    "evaluate_experiment": {"public": False, "writes": False},
    "pause_campaign": {"public": False, "writes": True},
    "pause_agent": {"public": False, "writes": True},
    "trigger_incident": {"public": False, "writes": True},
    "rollback_feature_flag": {"public": False, "writes": True},
    "generate_daily_report": {"public": False, "writes": True},
}
...
GROWTH_AGENT_DEFINITIONS = [
    {
        "id": "growth_commander",
        "name": "Growth Commander Agent",
        "instructions": "Coordinate the daily loop, keep agents within budget, and never bypass guardrails.",
        "allowed_tools": ["query_metrics", "generate_daily_report", "pause_agent", "trigger_incident"],
        "blocked_actions": ["send_customer_message", "publish_public_content", "change_pricing", "issue_refund"],
        "rate_limits": {"actions_per_day": 8},
    },
    {
        "id": "strategy",
        "name": "Strategy Agent",
        "instructions": "Select daily growth priorities from reader metrics, revenue, retention, and funnel health.",
        "allowed_tools": ["query_metrics", "evaluate_experiment", "create_campaign"],
        "blocked_actions": ["send_customer_message", "publish_public_content", "change_pricing"],
        "rate_limits": {"actions_per_day": 6},
    },
    {
        "id": "content",
        "name": "Content Agent",
        "instructions": "Draft literary, culturally sensitive campaign and social assets for admin review.",
        "allowed_tools": ["create_campaign_variant", "create_social_post_draft", "create_landing_page_draft"],
        "blocked_actions": ["publish_public_content", "send_customer_message"],
        "rate_limits": {"actions_per_day": 8},
    },
    {
        "id": "seo",
        "name": "SEO Agent",
        "instructions": "Draft search-safe metadata and content assets without keyword stuffing or fake claims.",
        "allowed_tools": ["query_metrics", "create_landing_page_draft", "create_campaign_variant"],
        "blocked_actions": ["publish_public_content", "hallucinate_availability"],
        "rate_limits": {"actions_per_day": 6},
    },
    {
        "id": "outreach",
        "name": "Outreach Agent",
        "instructions": "Prepare small, compliant outreach batches; frequency and consent gates are mandatory.",
        "allowed_tools": ["create_institution_lead", "send_institution_outreach", "schedule_email", "schedule_whatsapp"],
        "blocked_actions": ["spam", "send_without_guardrail"],
        "rate_limits": {"messages_per_day": GROWTH_OS_MAX_MESSAGES_PER_DAY},
    },
    {
        "id": "institution_sales",
        "name": "Institution Sales Agent",
        "instructions": "Score schools, colleges, libraries, and reading rooms for pilot outreach.",
        "allowed_tools": ["create_institution_lead", "send_institution_outreach", "update_crm_stage"],
        "blocked_actions": ["hallucinate_partnership", "promise_discount"],
        "rate_limits": {"messages_per_day": GROWTH_OS_MAX_INSTITUTION_OUTREACH_PER_DAY},
    },
    {
        "id": "creator_partnership",
        "name": "Creator Partnership Agent",
        "instructions": "Draft creator partnership opportunities with disclosed, accurate affiliate terms.",
        "allowed_tools": ["create_creator_affiliate_code", "schedule_email", "update_crm_stage"],
        "blocked_actions": ["hallucinate_partnership", "promise_payment"],
        "rate_limits": {"messages_per_day": GROWTH_OS_MAX_CREATOR_OUTREACH_PER_DAY},
    },
    {
        "id": "referral",
        "name": "Referral Agent",
        "instructions": "Prepare referral programs and anti-abuse checks before reward issuance.",
        "allowed_tools": ["create_referral_code", "issue_coupon", "query_metrics"],
        "blocked_actions": ["prohibited_discount", "unbounded_reward"],
        "rate_limits": {"actions_per_day": 10},
    },
    {
        "id": "customer_success",
        "name": "Customer Success Agent",
        "instructions": "Improve onboarding, first-read activation, and respectful reminders after meaningful engagement.",
        "allowed_tools": ["recommend_books", "start_reading_challenge", "send_reading_reminder", "create_support_ticket"],
        "blocked_actions": ["spam", "unsupported_availability_claim"],
        "rate_limits": {"messages_per_day": GROWTH_OS_MAX_MESSAGES_PER_DAY},
    },
    {
        "id": "support",
        "name": "Support Agent",
        "instructions": "Draft support replies and escalate payment, refund, or rights-sensitive cases.",
        "allowed_tools": ["answer_support_ticket", "create_support_ticket"],
        "blocked_actions": ["promise_refund", "change_pricing", "legal_advice"],
        "rate_limits": {"actions_per_day": 25},
    },
    {
        "id": "analytics",
        "name": "Analytics Agent",
        "instructions": "Summarize metrics, anomalies, and experiment outcomes from first-party data.",
        "allowed_tools": ["query_metrics", "evaluate_experiment", "generate_daily_report"],
        "blocked_actions": ["send_customer_message", "publish_public_content"],
        "rate_limits": {"actions_per_day": 12},
    },
    {
        "id": "trust_compliance",
        "name": "Trust and Compliance Agent",
        "instructions": "Review all public/customer-facing outputs for copyright, privacy, cultural, and claims safety.",
        "allowed_tools": ["query_metrics", "pause_campaign", "trigger_incident"],
        "blocked_actions": ["bypass_guardrail"],
        "rate_limits": {"actions_per_day": 40},
    },
    {
        "id": "availability",
        "name": "Availability Agent",
        "instructions": "Watch availability and performance signals; pause risky launches during incidents.",
        "allowed_tools": ["query_metrics", "trigger_incident", "rollback_feature_flag", "pause_campaign"],
        "blocked_actions": ["ignore_incident"],
        "rate_limits": {"actions_per_day": 20},
    },
    {
        "id": "finance_guard",
        "name": "Finance Guard Agent",
        "instructions": "Enforce budgets, discount policy, pricing consistency, and refund/payment promise safety.",
        "allowed_tools": ["query_metrics", "pause_campaign", "generate_daily_report"],
        "blocked_actions": ["change_pricing", "issue_refund", "promise_discount"],
        "rate_limits": {"actions_per_day": 20},
    },
]


GROWTH_GUARDRAIL_RULES = [
    {"id": "copyright_license", "label": "Copyright and licensing risk", "severity": "block"},
    {"id": "false_claims", "label": "False or exaggerated claims", "severity": "block"},
    {"id": "spam_frequency", "label": "Spam-like messaging or excessive frequency", "severity": "block"},
    {"id": "children_safety", "label": "Children/student appropriateness", "severity": "escalate_to_admin_queue"},
    {"id": "cultural_sensitivity", "label": "Political, religious, and cultural sensitivity", "severity": "escalate_to_admin_queue"},
    {"id": "privacy", "label": "Privacy and personal data exposure", "severity": "block"},
    {"id": "pricing", "label": "Pricing, refund, payment, or discount risk", "severity": "block"},
    {"id": "book_availability", "label": "Unsupported book availability claims", "severity": "block"},
    {"id": "testimonials", "label": "Hallucinated testimonials", "severity": "block"},
    {"id": "partnerships", "label": "Hallucinated institutional or creator partnerships", "severity": "block"},
    {"id": "budget", "label": "Budget overrun", "severity": "block"},
]
...
async def initialize_database_indexes() -> None:
    ...
    await db.campaign_variants.create_index([("campaign_id", 1), ("created_at", -1)])
    await db.experiments.create_index([("status", 1), ("created_at", -1)])
    await db.leads.create_index([("type", 1), ("stage", 1), ("created_at", -1)])
    await db.institutions.create_index([("stage", 1), ("score", -1), ("created_at", -1)])
    await db.creators.create_index([("stage", 1), ("created_at", -1)])
    await db.outreach_sequences.create_index([("status", 1), ("created_at", -1)])
    await db.messages.create_index([("channel", 1), ("status", 1), ("created_at", -1)])
    await db.support_tickets.create_index([("status", 1), ("created_at", -1)])
    await db.agent_runs.create_index([("agent", 1), ("created_at", -1)])
    await db.agent_runs.create_index([("status", 1), ("created_at", -1)])
    await db.agent_actions.create_index([("run_id", 1), ("created_at", -1)])
    await db.agent_actions.create_index([("status", 1), ("created_at", -1)])
    await db.guardrail_results.create_index([("action_id", 1), ("created_at", -1)])
    await db.guardrail_results.create_index([("decision", 1), ("created_at", -1)])
    await db.tool_calls.create_index([("run_id", 1), ("created_at", -1)])
    await db.incidents.create_index([("status", 1), ("severity", 1), ("created_at", -1)])
    await db.budgets.create_index([("scope", 1), ("date", 1)], unique=True)
    await db.metrics_snapshots.create_index([("date", -1), ("scope", 1)])
...
async def _run_startup_database_maintenance_once() -> None:
    ...
    for key, enabled in {
        ...
        "growth_os_public_execution": GROWTH_OS_PUBLIC_EXECUTION_ENABLED,
        "growth_os_emergency_pause": False,
    }.items():
        await db.feature_flags.update_one(
            {"key": key},
            {"$setOnInsert": {"key": key, "enabled": bool(enabled), "created_at": now_iso()}},
            upsert=True,
        )
...
def _growth_agent(agent_id: str) -> Optional[dict]:
    return next((agent for agent in GROWTH_AGENT_DEFINITIONS if agent["id"] == agent_id), None)


async def _growth_flag_enabled(key: str, default: bool = False) -> bool:
...
async def _growth_os_is_paused() -> bool:
    enabled = await _growth_flag_enabled("growth_os_enabled", GROWTH_OS_ENABLED)
    emergency_pause = await _growth_flag_enabled("growth_os_emergency_pause", False)
    return not enabled or emergency_pause


async def _growth_audit(event_type: str, actor: str, payload: Dict[str, Any]) -> None:
    safe_payload = payload if isinstance(payload, dict) else {}
    await db.audit_logs.insert_one({
        "id": str(uuid.uuid4()),
        "event_type": f"growth_os.{event_type}",
        "actor": actor[:160],
        "payload": _safe_analytics_metadata(safe_payload),
        "created_at": now_iso(),
    })


def _growth_action_templates(selected_agents: List[str]) -> List[dict]:
    allowed = set(selected_agents or [agent["id"] for agent in GROWTH_AGENT_DEFINITIONS])
    candidates = [
        {
            "agent": "analytics",
            "tool": "query_metrics",
            "title": "Summarize yesterday's reader funnel",
            "payload": {"metrics": ["paid_readers", "reading_sessions", "referrals", "payment_failed"]},
        },
        {
            "agent": "strategy",
            "tool": "create_campaign",
            "title": "Draft today's reading-time access priority",
            "payload": {"theme": "Where Learning Becomes Earning", "audience": "new readers"},
        },
        {
            "agent": "content",
            "tool": "create_social_post_draft",
            "title": "Draft premium social post for digital library onboarding",
            "payload": {"channel": "linkedin", "claim": "draft only", "requires_admin_review": True},
        },
        {
            "agent": "seo",
            "tool": "create_landing_page_draft",
            "title": "Draft SEO content for Bengali books and audiobooks",
            "payload": {"keywords": ["Earnalism", "digital library", "Bengali books", "audiobooks"]},
        },
        {
            "agent": "outreach",
            "tool": "schedule_email",
            "title": "Prepare opt-in reactivation reminder batch",
            "payload": {"channel": "email", "max_messages": min(25, GROWTH_OS_MAX_MESSAGES_PER_DAY), "status": "draft"},
        },
        {
            "agent": "institution_sales",
            "tool": "create_institution_lead",
            "title": "Score institution reading-room lead",
            "payload": {"lead_type": "library", "stage": "draft_scored"},
        },
        {
            "agent": "creator_partnership",
            "tool": "create_creator_affiliate_code",
            "title": "Draft creator partnership code for admin approval",
            "payload": {"status": "draft", "public_claim": "none"},
        },
        {
            "agent": "referral",
            "tool": "create_referral_code",
            "title": "Prepare referral code with anti-abuse review",
            "payload": {"reward": "reading_minutes", "anti_abuse_required": True},
        },
        {
            "agent": "customer_success",
            "tool": "start_reading_challenge",
            "title": "Draft seven-day first-read challenge",
            "payload": {"audience": "new_paid_readers", "status": "draft"},
        },
        {
            "agent": "support",
            "tool": "create_support_ticket",
            "title": "Create dormant-reader support review queue",
            "payload": {"reason": "no_first_reading_session", "status": "draft"},
        },
        {
            "agent": "trust_compliance",
            "tool": "pause_campaign",
            "title": "Review pending public actions before launch",
            "payload": {"review_required": True, "execution": "blocked_until_passed"},
        },
        {
            "agent": "availability",
            "tool": "query_metrics",
            "title": "Check uptime and reader latency before growth sends",
            "payload": {"metrics": ["health", "reader_latency", "payment_errors"]},
        },
        {
            "agent": "finance_guard",
            "tool": "generate_daily_report",
            "title": "Check daily spend and discount exposure",
            "payload": {"daily_budget_inr": GROWTH_OS_DAILY_BUDGET_INR},
        },
    ]
    return [item for item in candidates if item["agent"] in allowed]


def _growth_guardrail_decision(action: dict, mode: str) -> dict:
    reasons: List[str] = []
    decision = "allow"
    tool = action.get("tool", "")
    payload = action.get("payload") if isinstance(action.get("payload"), dict) else {}
    serialized = _json.dumps(payload, ensure_ascii=False, default=str).lower()

    if mode != "dry_run" and (GROWTH_OS_DRY_RUN_ONLY or not GROWTH_OS_PUBLIC_EXECUTION_ENABLED):
        decision = "block"
        reasons.append("Production execution is disabled by GROWTH_OS_DRY_RUN_ONLY/GROWTH_OS_PUBLIC_EXECUTION_ENABLED.")
    if tool not in GROWTH_TOOL_DEFINITIONS:
        decision = "block"
        reasons.append("Tool is not registered in the Growth OS tool registry.")
    if tool in {"schedule_email", "schedule_whatsapp", "schedule_social_post", "publish_landing_page", "answer_support_ticket", "send_reading_reminder", "send_institution_outreach"}:
        if mode == "production":
            if decision != "block":
                decision = "escalate_to_admin_queue"
                reasons.append("Customer-facing/public action requires admin queue approval.")
        else:
            decision = "revise" if decision == "allow" else decision
            reasons.append("Customer-facing/public action remains draft-only in dry-run mode.")
    if any(term in serialized for term in ["guaranteed", "official partner", "testimonial", "refund guaranteed", "free forever"]):
        decision = "block"
        reasons.append("Potential false, testimonial, partnership, refund, or discount claim detected.")
    if "price" in serialized or "discount" in serialized or tool == "issue_coupon":
        decision = "block"
        reasons.append("Pricing, coupon, and discount actions require separate finance approval.")

    return {
        "decision": decision,
        "reasons": reasons or ["All configured hard-policy gates passed."],
        "rules_checked": [rule["id"] for rule in GROWTH_GUARDRAIL_RULES],
    }


async def _growth_metrics_snapshot() -> dict:
    day = _utc_day()
    users = await db.users.count_documents({"role": "user"})
    paid_intents = await db.topup_intents.count_documents({"status": "credited"})
    reading_sessions = await db.reading_sessions.count_documents({})
    referrals = await db.analytics_events.count_documents({"event": {"$in": ["referral_invited", "referral_converted"]}})
    payments_failed = await db.topup_intents.count_documents({"status": "failed"})
    snapshot = {
        "id": str(uuid.uuid4()),
        "scope": "growth_os_daily",
        "date": day,
        "users": users,
        "paid_readers": paid_intents,
        "reading_sessions": reading_sessions,
        "referrals": referrals,
        "payment_failed": payments_failed,
        "created_at": now_iso(),
    }
    await db.metrics_snapshots.update_one(
        {"scope": snapshot["scope"], "date": day},
        {"$set": snapshot},
        upsert=True,
    )
    return snapshot
...
async def _run_growth_agent_action(run: dict, action: dict, mode: str) -> dict:
    now = now_iso()
    action_id = str(uuid.uuid4())
    guardrail = _growth_guardrail_decision(action, mode)
    status = "approved_draft" if guardrail["decision"] in {"allow", "revise"} else guardrail["decision"]
    action_doc = {
        "id": action_id,
        "run_id": run["id"],
        "agent": action["agent"],
    ...
    guardrail_doc = {
        ...
        "rules_checked": guardrail["rules_checked"],
        "created_at": now,
    }
    tool_doc = {
        "id": str(uuid.uuid4()),
        ...
        "status": "dry_run" if mode == "dry_run" else "blocked_until_approved",
        "created_at": now,
    }
    await db.agent_actions.insert_one(action_doc)
    await db.guardrail_results.insert_one(guardrail_doc)
    await db.tool_calls.insert_one(tool_doc)
    if guardrail["decision"] in {"block", "escalate_to_admin_queue"}:
        await db.analytics_events.insert_one({
            "id": str(uuid.uuid4()),
            "event": "agent_action_blocked" if guardrail["decision"] == "block" else "guardrail_triggered",
            "metadata": {"agent": action["agent"], "tool": action["tool"], "decision": guardrail["decision"]},
            "principal_role": "system",
            "principal_id": "growth_os",
            "path": "growth_os_daily_loop",
            "user_agent": "growth_os",
            "created_at": now,
        })
    return {**action_doc, "guardrail": guardrail_doc, "tool_call": tool_doc}


async def _growth_daily_loop(payload: GrowthRunIn, admin_email: str) -> dict:
    if payload.mode not in {"dry_run", "production"}:
        raise HTTPException(status_code=400, detail="mode must be dry_run or production")
    if await _growth_os_is_paused():
        raise HTTPException(status_code=423, detail="Growth OS is paused by feature flag or emergency switch")
    if payload.mode == "production" and (GROWTH_OS_DRY_RUN_ONLY or not await _growth_flag_enabled("growth_os_public_execution", GROWTH_OS_PUBLIC_EXECUTION_ENABLED)):
        raise HTTPException(status_code=403, detail="Growth OS production execution is disabled")

    selected = [agent for agent in payload.selected_agents if _growth_agent(agent)]
    templates = _growth_action_templates(selected)[:max(1, min(payload.max_actions, GROWTH_OS_MAX_ACTIONS_PER_RUN))]
    snapshot = await _growth_metrics_snapshot()
    run = {
        "id": str(uuid.uuid4()),
        "agent": "growth_commander",
        "objective": (payload.objective or "daily_growth_loop")[:160],
        "mode": payload.mode,
        "status": "running",
        "selected_agents": selected or [agent["id"] for agent in GROWTH_AGENT_DEFINITIONS],
        "metrics_snapshot_id": snapshot["id"],
        "budget_inr": GROWTH_OS_DAILY_BUDGET_INR,
    ...
    for action in templates:
        actions.append(await _run_growth_agent_action(run, action, payload.mode))

    blocked = sum(1 for item in actions if item["guardrail"]["decision"] == "block")
    escalated = sum(1 for item in actions if item["guardrail"]["decision"] == "escalate_to_admin_queue")
    status = "completed_with_blocks" if blocked or escalated else "completed"
    completed_at = now_iso()
    await db.agent_runs.update_one(
        {"id": run["id"]},
        {"$set": {"status": status, "completed_at": completed_at, "blocked_actions": blocked, "escalated_actions": escalated}},
    )
    run.update({"status": status, "completed_at": completed_at, "blocked_actions": blocked, "escalated_actions": escalated})
    await _growth_audit("daily_loop_run", admin_email, {"run_id": run["id"], "mode": payload.mode, "status": status})
    return {"ok": True, "run": run, "metrics_snapshot": snapshot, "actions": actions}
...
@api.get("/admin/growth/overview")
async def admin_growth_overview(_=Depends(require_admin)):
    flags = await db.feature_flags.find(
        {"key": {"$regex": "^growth_os"}},
        {"_id": 0},
    ).sort("key", 1).to_list(100)
    recent_runs = await db.agent_runs.find({}, {"_id": 0}).sort("created_at", -1).to_list(20)
    recent_actions = await db.agent_actions.find({}, {"_id": 0}).sort("created_at", -1).to_list(40)
    recent_guardrails = await db.guardrail_results.find({}, {"_id": 0}).sort("created_at", -1).to_list(40)
    incidents = await db.incidents.find({}, {"_id": 0}).sort("created_at", -1).to_list(20)
    latest_snapshot = await db.metrics_snapshots.find_one(
        {"scope": "growth_os_daily"},
        {"_id": 0},
        sort=[("date", -1)],
    )
    counters = {
        ...
        "blocked_actions": await db.guardrail_results.count_documents({"decision": "block"}),
        "escalated_actions": await db.guardrail_results.count_documents({"decision": "escalate_to_admin_queue"}),
        "incidents_open": await db.incidents.count_documents({"status": {"$ne": "resolved"}}),
        "campaigns": await db.campaigns.count_documents({}),
        "experiments": await db.experiments.count_documents({}),
        "institution_leads": await db.institutions.count_documents({}),
        "creator_leads": await db.creators.count_documents({}),
        "support_tickets": await db.support_tickets.count_documents({}),
    }
    return {
        "enabled": GROWTH_OS_ENABLED,
        "dry_run_only": GROWTH_OS_DRY_RUN_ONLY,
        "public_execution_enabled": GROWTH_OS_PUBLIC_EXECUTION_ENABLED,
        "config": {
            "daily_budget_inr": GROWTH_OS_DAILY_BUDGET_INR,
            "max_actions_per_run": GROWTH_OS_MAX_ACTIONS_PER_RUN,
            "max_messages_per_day": GROWTH_OS_MAX_MESSAGES_PER_DAY,
            "max_social_posts_per_day": GROWTH_OS_MAX_SOCIAL_POSTS_PER_DAY,
            "max_landing_pages_per_day": GROWTH_OS_MAX_LANDING_PAGES_PER_DAY,
            "max_creator_outreach_per_day": GROWTH_OS_MAX_CREATOR_OUTREACH_PER_DAY,
            "max_institution_outreach_per_day": GROWTH_OS_MAX_INSTITUTION_OUTREACH_PER_DAY,
        },
        "agents": GROWTH_AGENT_DEFINITIONS,
        "tools": GROWTH_TOOL_DEFINITIONS,
        "guardrails": GROWTH_GUARDRAIL_RULES,
        "feature_flags": flags,
        "counters": counters,
...
@api.post("/admin/growth/run-daily")
async def admin_growth_run_daily(payload: GrowthRunIn, admin=Depends(require_admin)):
    return await _growth_daily_loop(payload, admin.get("email", "admin"))


@api.post("/admin/growth/kill-switch")
async def admin_growth_kill_switch(payload: GrowthKillSwitchIn, admin=Depends(require_admin)):
    key = re.sub(r"[^a-zA-Z0-9_.:-]", "_", payload.key.strip())[:80]
    if not key.startswith("growth_os"):
        raise HTTPException(status_code=400, detail="Only growth_os feature flags can be changed here")
    now = now_iso()
    await db.feature_flags.update_one(
        {"key": key},
        {"$set": {
    ...
    await _growth_audit(
        ...
        {"key": key, "enabled": bool(payload.enabled), "reason": payload.reason[:240]},
    )
    return {"ok": True, "key": key, "enabled": bool(payload.enabled)}
