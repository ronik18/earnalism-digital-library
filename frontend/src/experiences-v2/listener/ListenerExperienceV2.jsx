...
export function clampPreviewTime(seconds, previewSeconds) {
  const value = Math.max(0, Number(seconds) || 0);
  return previewSeconds > 0 ? Math.min(value, previewSeconds) : value;
}
...
export default function ListenerExperienceV2({ book = {}, fixture = false, access = {}, onAuthorize, onPlaybackStateChange, onNavigate, onAddToLibrary, onReadAlong, readAlongSupported = false }) {
  ...
  const previewLimit = presentation.previewSeconds || 0;

  useEffect(() => () => { audioRef.current?.pause(); }, []);
  const seek = useCallback((next) => {
    const audio = audioRef.current;
    if (!audio) return;
    const safeTime = clampPreviewTime(next, previewLimit);
  ...
  const onTimeUpdate = useCallback(() => {
    const audio = audioRef.current;
    if (!audio) return;
    const safeTime = clampPreviewTime(audio.currentTime, previewLimit);
    if (safeTime !== audio.currentTime) { audio.pause(); audio.currentTime = safeTime; }
    setTime(safeTime);
  }, [previewLimit]);

  if (!presentation.canRender) return null;
  const effectiveDuration = previewLimit || duration;
    ...
    <ExperienceShell className="listener-v2" labelledBy="listener-v2-title">
      ...
      <section className="listener-v2__layout">
        <div className="listener-v2__main">
          ...
          {!presentation.fixture && access.authorized && <audio ref={audioRef} src={presentation.mediaUrl} preload="metadata" onLoadedMetadata={(event) => setDuration(Number(event.currentTarget.duration) || 0)} onTimeUpdate={onTimeUpdate} onPlay={() => { setPlaying(true); onPlaybackStateChange?.("playing"); }} onPause={() => { setPlaying(false); onPlaybackStateChange?.("paused"); }} />}
          <div className="listener-v2__timeline"><div><span>{clock(time)}</span><input aria-label="Seek within approved audiobook preview" type="range" min="0" max={effectiveDuration || 0} step="0.1" value={Math.min(time, effectiveDuration || time)} onChange={(event) => seek(event.target.value)} disabled={presentation.fixture || !effectiveDuration} /><b style={{ width: `${progress}%` }} aria-hidden="true" /><span>{clock(effectiveDuration)}</span></div><p>{presentation.chapterLabel}</p></div>
