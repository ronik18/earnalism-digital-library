...
export default function ReadingPassPaywall({
  ...
  const audiobookPreviewEnded = reason === 'AUDIO_PREVIEW_ENDED';
  const paused = reason === 'PAUSED' || reason === 'LEASE_EXPIRED' || audiobookPreviewEnded;
  ...
  return (
    <div className="reading-pass-paywall" role="presentation">
      ...
      <section ref={dialogRef} className="reading-pass-paywall__dialog" role="dialog" aria-modal="true" aria-labelledby="reading-pass-paywall-title" aria-describedby="reading-pass-paywall-description">
        ...
        <h2 id="reading-pass-paywall-title">{elsewhere ? 'Continue on this device?' : 'The free preview ends here.'}</h2>
        ...
        <div className="reading-pass-paywall__actions">
          {elsewhere || paused ? (
            <button type="button" className="reading-pass-paywall__primary" onClick={onTransfer}><RefreshCw size={18} />{elsewhere ? 'Continue on this device' : audiobookPreviewEnded ? 'Continue audiobook' : 'Resume reading'}</button>
          ) : (
          ...
          <button type="button" onClick={onPreview}><ArrowLeft size={18} />{audiobookPreviewEnded ? 'Replay free preview' : 'Return to free preview'}</button>
        </div>
      </section>
    </div>
  );
