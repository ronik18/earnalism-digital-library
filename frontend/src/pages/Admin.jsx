...
export function isCompleteReadingPassReleasePreflight(value) {
  ...
  if (!pointers || !["OBSERVED", "UNKNOWN"].includes(pointers.status) || !inspectionCount(pointers.limit) || typeof pointers.truncated !== "boolean" || !(typeof pointers.manifest_truncated === "boolean" || pointers.manifest_truncated === null) || !(pointers.duplicate_count === null || inspectionCount(pointers.duplicate_count)) || !(pointers.observed_count === null || inspectionCount(pointers.observed_count)) || !(pointers.valid_count === null || inspectionCount(pointers.valid_count)) || !(pointers.compatible_count === null || inspectionCount(pointers.compatible_count)) || !(pointers.incompatible_count === null || inspectionCount(pointers.incompatible_count)) || !["ALL_OBSERVED_POINTERS_MATCH_SINGLE_ACTIVE_MANIFEST", "INCOMPLETE_OR_INCONSISTENT_POINTER_MANIFEST_STATE", "NO_ACTIVE_POINTERS_OBSERVED", "UNKNOWN"].includes(pointers.compatibility)) return false;
    ...
    <section className="card-elegant p-6 sm:p-8 mt-8" data-testid="admin-reading-pass-release-preflight">
      ...
      {preflight && (
        <div className="mt-6 space-y-5 text-sm" data-testid="reading-pass-release-preflight-result">
          ...
          <div className="rounded-lg border border-brand-soft bg-white/55 p-4 text-charcoal-soft">Observation completeness: <span className="font-medium text-charcoal">{preflight.complete ? "complete bounded observation" : "partial or inconsistent; do not infer a pass"}</span>. {pointers.truncated ? "Pointer results were truncated." : pointers.manifest_truncated ? "Active-manifest results were truncated." : "Result bounds were not truncated."}</div>
