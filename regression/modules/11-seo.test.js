...
const STATIC_SNAPSHOT_ROUTES = PUBLIC_RELEASE_HELD
  ? ["/", "/library", "/pricing"]
  : ["/", "/book/dracula", "/library", "/pricing", "/reader/dracula"];
...
describe("Crawler-visible Dracula SEO snapshots", () => {
  ...
  const bookHtml = PUBLIC_RELEASE_HELD ? "" : readSnapshot("/book/dracula");
  const libraryHtml = readSnapshot("/library");
  const pricingHtml = readSnapshot("/pricing");
  const readerHtml = PUBLIC_RELEASE_HELD ? "" : readSnapshot("/reader/dracula");
  ...
  test("/book/dracula has crawler-visible Dracula-specific metadata", () => {
    if (PUBLIC_RELEASE_HELD) {
      ...
      expect(STATIC_SEO_PUBLIC_CONTRACT.publications).toEqual([]);
      expect(routes).not.toContain("/book/dracula");
      expect(routes).not.toContain("/reader/dracula");
      return;
    }
  ...
  test("homepage, library, and pricing snapshots preserve the canonical preview and release-truth contract", () => {
    ...
    expect(homeHtml).toContain(releaseCopy);
    expect(homeHtml).not.toMatch(/QA_PASSED|APPROVED/);
    expect(libraryHtml).toContain(releaseCopy);
    expect(pricingHtml).toContain(releaseCopy);
  ...
  test("sitemap and robots preserve the controlled SEO surface", () => {
    if (PUBLIC_RELEASE_HELD) {
      expect(sitemap).not.toMatch(/\/book\/|\/reader\//);
      expect(robots).not.toMatch(/Allow: \/reader\//);
  ...
  test("the frontend-local public SEO contract is fresh, data-driven, and contains no protected publication data", () => {
    ...
    if (PUBLIC_RELEASE_HELD) {
      expect(contract.public_release_held).toBe(true);
      expect(contract.publications).toEqual([]);
      return;
    }
