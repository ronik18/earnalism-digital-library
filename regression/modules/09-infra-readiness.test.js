const { apiOrigin, isProductionTarget, isGoLive, loadTestAllowed, productionGoLiveAllowed } = require("../utils/envGuard");
...
describe("Auto-Scaling & Infrastructure", () => {
  ...
  test("GO LIVE target is staging unless explicitly allowed", async () => {
    if (isGoLive()) {
      if (isProductionTarget()) {
        expect(productionGoLiveAllowed()).toBe(true);
      } else {
  ...
  test("load testing is gated away from production", async () => {
    ...
    } else if (isGoLive() && isProductionTarget()) {
      expect(loadTestAllowed()).toBe(false);
    } else if (isGoLive()) {
  ...
  test("Railway/Judoscale readiness secrets are present for GO LIVE", async () => {
    ...
    if (process.env.REGRESSION_REQUIRE_INFRA_SECRETS !== "true") {
      expect(true).toBe(true);
      return;
    }
