...
- The safe backend fix is not to fall back to static artifact audio. Instead, require explicit `audio_materialization_slugs` for runtime audio and merge controlled artifact source/provenance evidence into the DB book only for those slugs, preserving the admin-approved Sarvam/B2 audio URLs.
...
- Endpoint materialization can succeed while browser gate still fails if production frontend is stale. On 2026-07-07, backend deploy `9f39fb50-26e1-47a7-b339-91fe217cff3c` returned pilot detail audio enabled, manifest audio enabled, endpoint `206`, sidecars `200`, and non-pilot Bengali audio hidden, but the production Vercel bundle `main.601f9ab8.js` lacked reader-manifest audio markers and rendered no audio controls.
- Do not regenerate Sarvam audio for this blocker. The next action is frontend deploy/merge with Reader manifest-audio support, followed by metadata/browser-only final gate.
