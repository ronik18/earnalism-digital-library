...
- [ ] Representative and every required full-book listening sample score is `>= 9.4` with confidence `>= 0.9`.
- [ ] No robotic texture, mechanical cadence, list-reading rhythm, choppy joins, fallback TTS, or placeholder audio.
- [ ] Measured `paragraph_or_stanza` sync PASS; `auto_estimated_sync=false`.
