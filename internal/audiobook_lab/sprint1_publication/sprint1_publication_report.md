...
| `pather-panchali` | পথের পাঁচালী / Pather Panchali | Bengali | Yes | No | NOT_RUN | `internal/audiobook_lab/sprint1_publication/title_runs/pather-panchali_report.md` | $0.0000 | SPRINT_TARGET_INCOMPLETE |
...
| `dsires-baby` | Désirée's Baby | English | Yes | No | Studio-C representative scores `9.4, 8.4, 7.5, 9.4`; minimum confidence `0.85`; fatal robotic texture/mechanical cadence | `internal/audiobook_lab/sprint1_publication/title_runs/dsires-baby_release_gate_evidence.json` | $0.23616 conservative estimate; actual unknown | ALTERNATE_VOICE_AUDITION_REQUIRED |
...
## Parallel Yes+Yes Acceleration: Désirée's Baby

`dsires-baby` passed deterministic source preparation with 11,974 sanitized characters, one ready chapter, rights approval, and source SHA-256 `587455ed554ef64d19f0ea7dcd31940d242aa759f5132b6514b130efa4a64a89`. The historical Piper asset remains excluded because its per-voice commercial/speaker rights are on hold.

Stage 2H confirmed DNS, Google ADC, and OpenAI/Google credentials, then completed the source-bound Google Studio-C audition after the earlier network-only failure. Four samples synthesized for an estimated `$0.03616`; bounded listening QA added `$0.20`. Scores were `9.4`, `8.4`, `7.5`, and `9.4`, with minimum confidence `0.85`. The risk passage triggered fatal robotic-texture and mechanical-cadence flags, so the representative audition failed and this exact fingerprint must not repeat.

Full TTS, ASR, upload, release-state mutation, and publication did not run. The title remains reader-public/audio-hidden in `ALTERNATE_VOICE_AUDITION_REQUIRED`; the next bounded action is one materially different `en-GB-Chirp3-HD-Achird` audition, followed by human narration or licensed-audio escalation if that arm fails. Stage 2H conservatively estimates `$0.23616`, bringing the Sprint checkpoint to `$10.41276 / $175`; actual billing remains unreported and the lock restored byte-for-byte.

The approved-audio regression lane also found stale D19 controlled-publication fields that contradicted its failed QA state. PR #116 merged the fail-closed root/backend packets and release-truth guards as `a682215c6ee66fd2b64d77874f189c70818d334c`. Reader access remains preserved, audio assets/provider metadata remain empty, and no direct audiobook URL is exposed.

## Autonomous V2 Closeout

The coordinator bound the owner-approved `$175` Sprint cap and `$30` per-title cap into every serialized paid wrapper. It processed `the-cop-and-the-anthem`, `the-last-leaf`, `the-masque-of-the-red-death`, `dsires-baby`, `the-necklace`, `the-monkeys-paw`, and `the-yellow-wallpaper`. Google Studio-C and Chirp3-HD-Achird were not repeated after failed fingerprints.

Six titles failed representative listening QA across both voice families and stopped before full TTS. `the-monkeys-paw` was the only title to pass representative QA and reach a private full candidate: raw ASR/source passed at `9.8333`, first/last checks passed, but one listening sample scored `7.5` with robotic/mechanical fatal flags. One bounded ending repair improved that local sample, yet full-candidate revalidation still scored `8.4` on two samples. It was not published.

All seven processed titles now have source-bound human-narration/licensed-import packets and exact received-audio validation commands. Public audiobook truth remains exactly `book-2b9853ec52` and `a-ghost-story`; no manifest, endpoint, frontend release state, upload, or production deployment changed. Conservative estimated spend is `$14.90614 / $175`, leaving `$160.09386`; actual provider billing remains unknown.
