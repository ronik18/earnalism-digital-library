...
The first source-bound Google Studio-C audition was budgeted at `$0.03616` for four representative passages. The execution environment could not resolve `texttospeech.googleapis.com`, so synthesis never started: `provider_calls_ran=false`, `synthesis_calls=0`, no audio or listening evidence exists, and `$0.00` is booked. The lock restored byte-for-byte and public audio remains hidden. This is a retryable infrastructure state, not a provider-quality failure; the exact network-enabled retry command is recorded in `title_runs/dsires-baby_report.md`.

The approved-audio regression lane also found stale D19 controlled-publication fields that contradicted its failed QA state. Both root and backend packets are now prepared fail-closed with reader access preserved, `PUBLIC_AUDIO_RELEASE_NOT_APPROVED`, empty audio assets/provider metadata, updated checksums, and no direct audiobook URL. Focused backend release-truth tests pass `35/35`; publication behavior remains unchanged until the focused PR is merged and deployed.
