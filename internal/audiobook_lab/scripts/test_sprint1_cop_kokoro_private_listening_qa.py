...
MODULE_PATH = Path(__file__).with_name("sprint1_cop_kokoro_private_listening_qa.py")
SPEC = importlib.util.spec_from_file_location("sprint1_cop_kokoro_private_listening_qa", MODULE_PATH)
assert SPEC and SPEC.loader
qa = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(qa)
ACTUAL_EVIDENCE_SHA256 = qa.EXPECTED_EVIDENCE_SHA256
ACTUAL_SOURCE_SHA256 = qa.EXPECTED_SOURCE_SHA256
ACTUAL_ATTEMPT_FINGERPRINT = qa.EXPECTED_ATTEMPT_FINGERPRINT
ACTUAL_ASR_CONFIG_FINGERPRINT = qa.EXPECTED_ASR_CONFIG_FINGERPRINT
ACTUAL_SAMPLE_BINDINGS = qa.EXPECTED_SAMPLE_BINDINGS


class CopKokoroPrivateListeningQATests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.private_temporary = tempfile.TemporaryDirectory(
            prefix="test-cop-kokoro-listening-",
        ...
        for index, passage_id in enumerate(
            ("opening_winter", "waiter_dialogue", "church_reckoning", "ironic_ending")
        ):
    ...
    def test_default_contract_is_bound_to_exact_current_cop_evidence(self) -> None:
        self.assertEqual(qa.SLUG, "the-cop-and-the-anthem")
        self.assertEqual(
            ACTUAL_EVIDENCE_SHA256,
            "6d1ea462d37c72e71a92ec7267ec453b089838ec1ee78b30c1cc61363efd6530",
        )
        self.assertEqual(
            ACTUAL_SOURCE_SHA256,
            "77a6d1c7ff6162cc7aad47b950666c6bb1dedf0beb55a08f3f476e27e57bd3ab",
        )
        self.assertEqual(
            ACTUAL_ATTEMPT_FINGERPRINT,
            "b693be6196019d6e44b22ac1cdcc7e1ea7099550f69eb326bb3cdacf4f27c6bf",
        )
        self.assertEqual(
            ACTUAL_ASR_CONFIG_FINGERPRINT,
            "3610d60e9094c3f80f4c22d65b63db45f622f48b7ea927301acf01a221444075",
        )
        self.assertEqual(
            {
                passage_id: binding["audio_sha256"]
                for passage_id, binding in ACTUAL_SAMPLE_BINDINGS.items()
            },
            {
    ...
    def test_asr_score_or_order_drift_fails_closed(self) -> None:
        ...
        with mock.patch.object(qa, "EXPECTED_EVIDENCE_SHA256", qa.sha256_file(self.evidence)):
            with self.assertRaisesRegex(qa.CopKokoroListeningQAError, "ASR score"):
    ...
    def test_platform_pass_without_exact_ten_blocks_next_stage(self) -> None:
        ...
        self.assertEqual(result["status"], "PRIVATE_COP_PLATFORM_PASS_OWNER_EXACT_10_NOT_MET")
        self.assertEqual(self.lock.read_bytes(), self.original_lock)
    ...
    def test_exact_ten_pass_still_is_not_release_evidence(self) -> None:
        ...
        self.assertEqual(len(result["judged_samples"]), 4)
    ...
    def test_frontmatter_blocks_even_with_exact_scores(self) -> None:
        ...
        self.assertIn("opening_winter: FRONTMATTER_PRESENT", gate["sample_blockers"])
    ...
    def test_budget_cap_must_be_exact_and_provider_error_restores_lock(self) -> None:
        ...
        self.assertEqual(code, 3)
        self.assertEqual(result["status"], "PRIVATE_COP_LISTENING_QA_ERROR")
    ...
    def test_public_output_path_is_rejected(self) -> None:
        with self.assertRaisesRegex(qa.CopKokoroListeningQAError, "public output path"):
            qa.execute(
                self.evidence,
                qa.ROOT / "frontend/public/audio/cop.json",
                self.lock,
                dry_run=True,
                env=self.env,
            )
