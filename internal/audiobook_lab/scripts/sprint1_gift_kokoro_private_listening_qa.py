#!/usr/bin/env python3
"""Judge the exact Gift of the Magi Kokoro representative candidate.
...
AUTHOR = "O. Henry"
LANGUAGE = "eng"
SCOPE = "gift_kokoro_representative_listening_qa_v1"
HOLDER = "sprint1_gift_kokoro_private_listening_qa"
EXPECTED_SAMPLE_COUNT = 4
EXPECTED_SCHEMA = "earnalism.kokoro_title_representative.v1"
EXPECTED_STATUS = "REPRESENTATIVE_OBJECTIVE_PASS_AWAITING_INDEPENDENT_LISTENING_QA"
EXPECTED_EVIDENCE_SHA256 = "7c0aa1286d5915899afa1b2eafc2afb7d79512ea14548c8ec490f570be217dde"
EXPECTED_SOURCE_SHA256 = "be7f050f1affc65144172ae7157ad10ab8a8ee698e196623ff072fe410f4ec5e"
EXPECTED_ATTEMPT_FINGERPRINT = "bf5ff8b24d23e4ae912d332b247d26d5efb60eeb0b91ebad0bc179e40a7ea015"
EXPECTED_ASR_CONFIG_FINGERPRINT = "1b932d7d1193947aad72e51cc2deba889a4c09468b21618d02b21b38db124755"
...
EXPECTED_SAMPLE_BINDINGS: dict[str, dict[str, Any]] = {
    "opening_money": {
        ...
        "audio_sha256": "5f2c6c4e571eb41bc7a974eb6ba9fcbcb782f374bfd15ec644c76f88adb7f6c6",
        "size_bytes": 1_138_844,
        "duration_seconds": 23.725,
    },
    "hair_sale_dialogue": {
        "source_text_sha256": "033d48a6a7b89a0341ad37b7abe0d3495a8ddd1c3dfa1b1cd03c20d3ef4db61d",
        "audio_sha256": "188da386380ac30e4aad451522869a8bc616d2d8c03ff4b08451484088bc8461",
        "size_bytes": 932_444,
        "duration_seconds": 19.425,
    },
    "sacrifice_dialogue": {
        "source_text_sha256": "6a5f6a372c472a7b43e4fa56178e53a74861c3847f7043469dd3958892bceeac",
        "audio_sha256": "ba959e59fafd7df838bde8f1330976cda98360f1a92c06a22ed43f15d582e3df",
    ...
    "magi_ending": {
        ...
        "audio_sha256": "8124feb940e30bf675342ef9cb6d2d8ac29f885bf2cedd333853309af6a27f0a",
        "size_bytes": 1_975_244,
...
EXPECTED_ENV = {
    "EARNALISM_APPROVE_GIFT_KOKORO_LISTENING_QA": "true",
...
class GiftKokoroListeningQAError(RuntimeError):
...
def assert_nonpublic_output(path: Path) -> None:
    ...
    if any(marker in rendered for marker in forbidden):
        raise GiftKokoroListeningQAError(f"public output path is forbidden: {path}")
...
def assert_private_audio(path: Path) -> Path:
    ...
    if "/internal/audiobook_lab/private_runs/" not in rendered:
        raise GiftKokoroListeningQAError(f"audio is outside private_runs: {resolved}")
    if any(marker in rendered for marker in ("/frontend/public/", "/frontend/build/", "/public/audio/")):
        raise GiftKokoroListeningQAError(f"public audio path is forbidden: {resolved}")
    return resolved
...
def _require_equal(observed: Any, expected: Any, label: str) -> None:
    if observed != expected:
        raise GiftKokoroListeningQAError(f"{label} changed: expected {expected!r}, observed {observed!r}")


def load_evidence(path: Path) -> tuple[dict[str, Any], list[dict[str, Any]], str]:
    if not path.is_file():
        raise GiftKokoroListeningQAError(f"Gift evidence is missing: {path}")
    ...
    for passage_id, expected in EXPECTED_SAMPLE_BINDINGS.items():
        ...
        if not audio.is_file():
            raise GiftKokoroListeningQAError(f"private audio is missing: {passage_id}")
        ...
        if abs(measured_duration - float(expected["duration_seconds"])) > 0.005:
            raise GiftKokoroListeningQAError(
...
def load_lock(raw: bytes) -> dict[str, Any]:
    try:
        ...
        raise GiftKokoroListeningQAError(f"paid_tts.lock is invalid JSON: {exc}") from exc
    if payload.get("status") != "active" or payload.get("current_holder") != "none":
        raise GiftKokoroListeningQAError("paid_tts.lock is not safely available")
    if payload.get("allowed_next_holders") != []:
        raise GiftKokoroListeningQAError("paid_tts.lock allowed_next_holders must be empty")
    return payload
...
def execute(
    ...
    *,
    ...
    preflight: dict[str, Any] = {
        ...
        "scope": "PRIVATE_GIFT_REPRESENTATIVE_SCREEN_ONLY_NOT_RELEASE_EVIDENCE",
    ...
    try:
        ...
        if lock_path.read_bytes() != original_lock:
            raise GiftKokoroListeningQAError("paid_tts.lock changed before acquisition")
        atomic_write(
            lock_path,
        ...
        if acquired.get("current_holder") != HOLDER or acquired.get("allowed_slugs") != [SLUG]:
            raise GiftKokoroListeningQAError("paid_tts.lock acquisition scope mismatch")
    ...
    if error:
        status, code = "PRIVATE_GIFT_LISTENING_QA_ERROR", 3
    elif gate["owner_exact_10_pass"]:
        status, code = "PRIVATE_GIFT_REPRESENTATIVE_EXACT_10_PASS_NOT_RELEASE_EVIDENCE", 0
    elif gate["platform_screen_pass"]:
        status, code = "PRIVATE_GIFT_PLATFORM_PASS_OWNER_EXACT_10_NOT_MET", 5
    else:
        status, code = "PRIVATE_GIFT_LISTENING_QA_BLOCKED", 3
    result = {
        **preflight,
        "status": status,
