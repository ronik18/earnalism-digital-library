#!/usr/bin/env python3
"""Focused tests for Monkey's Paw exact four-sample listening QA."""
...
SCRIPT = Path(__file__).with_name(
    "sprint1_monkeys_paw_bf_emma_private_listening_qa.py"
)
SPEC = importlib.util.spec_from_file_location("monkeys_paw_listening", SCRIPT)
...
class MonkeysPawListeningQATests(unittest.TestCase):
    def test_evidence_binds_four_exact_private_wavs(self) -> None:
        evidence, samples, fingerprint = MODULE.load_evidence(
    ...
    def test_runtime_and_budget_guards_are_exact(self) -> None:
        env = approved_env()
        self.assertEqual(BASE.runtime_gate_errors(env), [])
        budget = BASE.budget_guard(env)
        self.assertEqual(budget["status"], "PASS")
        self.assertEqual(budget["estimated_listening_qa_usd"], 0.2)
        broken = dict(env)
        broken["MAX_TTS_BUDGET_USD"] = "1.00"
        self.assertEqual(BASE.budget_guard(broken)["status"], "BLOCKED")

    def test_exact_ten_is_required_for_next_private_stage(self) -> None:
        exact = BASE.evaluate_judgments(
            [judgment({"sample_label": str(index)}) for index in range(4)]
        )
        self.assertIs(exact["platform_screen_pass"], True)
        self.assertIs(exact["owner_exact_10_pass"], True)
        self.assertIs(exact["next_private_stage_authorized"], True)
        ...
        self.assertIs(platform_only["owner_exact_10_pass"], False)
        self.assertIs(platform_only["next_private_stage_authorized"], False)
        fatal = BASE.evaluate_judgments(
            [judgment({"sample_label": str(index)}, fatal=index == 0) for index in range(4)]
        )
        self.assertIs(fatal["platform_screen_pass"], False)
    ...
    def test_mocked_exact_ten_restores_lock_and_never_publishes(self) -> None:
        ...
        self.assertEqual(
            result["status"],
            "PRIVATE_MONKEYS_PAW_BF_EMMA_REPRESENTATIVE_EXACT_10_PASS_NOT_RELEASE_EVIDENCE",
        )
    ...
    def test_source_has_no_generation_upload_or_publication_path(self) -> None:
    ...
    def test_cli_surface_is_listening_only(self) -> None:
        ...
        self.assertIn("--paid-lock", result.stdout)
