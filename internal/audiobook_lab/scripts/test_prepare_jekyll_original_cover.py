...
SCRIPT_PATH = Path(__file__).with_name("prepare_jekyll_original_cover.py")
SPEC = importlib.util.spec_from_file_location("prepare_jekyll_original_cover", SCRIPT_PATH)
assert SPEC and SPEC.loader
MODULE = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(MODULE)

GENERATED_AT = "2026-07-30T13:15:00Z"
...
def test_original_pair_has_no_external_art_or_territorial_blocker(tmp_path: Path) -> None:
...
def test_output_geometry_performance_and_public_copy_are_safe(tmp_path: Path) -> None:
    package = tmp_path / "candidate"
    prepare(tmp_path, "candidate")
    validation = MODULE.LEGACY.read_json(package / "visual_validation.json")
    ...
    assert validation["checks"]["territorial_rights_blocker"] is False
...
def test_controlled_catalog_remains_byte_identical(tmp_path: Path) -> None:
    before = {
        ...
        for path in MODULE.LEGACY.CATALOG_PATHS
    }
    manifest = prepare(tmp_path, "candidate")
    after = {
        path: MODULE.sha256_file(path)
...
def test_committed_manual_review_is_bound_to_exact_original_outputs() -> None:
    package = MODULE.DEFAULT_PACKAGE
    manifest = MODULE.LEGACY.read_json(package / "candidate_manifest.json")
    review = MODULE.LEGACY.read_json(package / "manual_visual_review.json")

    assert review["decision"]["private_candidate_visual_gate"] == "PASS"
    assert review["decision"]["admin_upload_authorized"] is True
    assert (
        review["decision"]["canonical_promotion_authorized_after_remote_checksum_match"]
        is True
    )
