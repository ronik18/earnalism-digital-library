...
def openai_listening_qa_budget_guard(*, sample_count: int = 1, total_estimated_usd: float = 0.0) -> dict:
    """Fail closed before OpenAI listening-QA calls unless bounded by env caps."""
    ...
    estimate_raw = os.environ.get(OPENAI_LISTENING_QA_ESTIMATED_USD_ENV, "").strip()
    if estimate_raw:
        try:
            per_sample_estimate = float(estimate_raw)
        except ValueError:
            return {
                "ok": False,
                "code": "LISTENING_QA_BUDGET_GATE_INVALID",
                "blocker": f"LISTENING_QA_BUDGET_GATE_INVALID: {OPENAI_LISTENING_QA_ESTIMATED_USD_ENV} must be numeric.",
                "cap_env": OPENAI_LISTENING_QA_MAX_ESTIMATED_USD_ENV,
                "estimate_env": OPENAI_LISTENING_QA_ESTIMATED_USD_ENV,
            }
        if per_sample_estimate < 0:
            return {
                "ok": False,
                "code": "LISTENING_QA_BUDGET_GATE_INVALID",
                "blocker": f"LISTENING_QA_BUDGET_GATE_INVALID: {OPENAI_LISTENING_QA_ESTIMATED_USD_ENV} must be nonnegative.",
                "cap_env": OPENAI_LISTENING_QA_MAX_ESTIMATED_USD_ENV,
                "estimate_env": OPENAI_LISTENING_QA_ESTIMATED_USD_ENV,
            }
    else:
        per_sample_estimate = OPENAI_LISTENING_QA_DEFAULT_ESTIMATED_USD
    count = max(int(sample_count or 1), 1)
    qa_estimate = round(per_sample_estimate * count, 4)
    total_estimate = round(safe_float(total_estimated_usd, 0.0) + qa_estimate, 4)
    if cap is None:
        return {
            "ok": False,
    ...
    if total_estimate > total_cap:
        return {
            ...
            "blocker": f"LISTENING_QA_TOTAL_BUDGET_EXCEEDED: estimated TTS plus listening QA cost ${total_estimate:.4f} exceeds total cap ${total_cap:.4f}.",
            "cap_env": OPENAI_LISTENING_QA_MAX_ESTIMATED_USD_ENV,
            "estimated_qa_cost_usd": qa_estimate,
            "listening_qa_cap_usd": cap,
...
def frontmatter_absent(text: str) -> bool:
    banned = ["project gutenberg", "gutenberg.org", "wikisource", "repository", "source:", "গল্পগুচ্ছ", "পৃষ্ঠা"]
    lowered = text.lower()
    if any(item in lowered for item in banned):
        return False
    if re.search(r"(?m)^\s*[০-৯\d]{3,4}\s*[।.!?]?\s*$", text):
        return False
    # Page references are frontmatter, but Bengali literary words such as
    # "পৃথিবী" and "পৃথগন্ন" must not be rejected because they start with পৃ.
    return re.search(r"(?:^|\s)পৃ(?:[.\sঃ:]|$)", text) is None
    ...
    if not budget_guard.get("ok"):
        return {
            **sample,
            "scores": {},
            "confidence": 0.0,
            "notes": budget_guard.get("blocker", ""),
            "blocker_reason": budget_guard.get("code", "LISTENING_QA_BUDGET_BLOCKED"),
