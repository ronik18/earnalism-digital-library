...
def matching_representative_pass(args) -> dict:
    ...
    return {
        "status": "BLOCKED",
        "blockers": [
            "No fresh representative audition evidence matches "
            f"{SARVAM_FULL_PILOT_SLUG} + Sarvam {SARVAM_FULL_PILOT_MODEL} + "
            f"{SARVAM_FULL_PILOT_VOICE} + {SARVAM_FULL_PILOT_STYLE} "
            "with score >=9.2/confidence >=0.90/no fatal flags."
        ],
    }
...
def sarvam_full_pilot_preflight(args, manuscript: str) -> dict:
    ...
    if not SARVAM_FULL_PILOT_VOICE:
        blockers.append("EARNALISM_BENGALI_TTS_VOICE is required.")
    ...
    try:
        ...
        budget = None
    total_budget_raw = os.environ.get("MAX_TTS_BUDGET_USD", "").strip()
    try:
        total_budget = float(total_budget_raw) if total_budget_raw else None
    except ValueError:
        total_budget = None
    estimate = estimated_sarvam_cost_usd(manuscript)
    if total_budget is None:
        blockers.append("MAX_TTS_BUDGET_USD is required.")
    if budget is None:
    ...
    if total_budget is not None and bool_env("EARNALISM_STOP_ON_BUDGET_EXCEEDED") and estimate > total_budget:
        blockers.append(f"Estimated Sarvam TTS cost ${estimate:.4f} exceeds total budget ${total_budget:.4f}.")
...
def bengali_tts_source_like_line(stripped: str) -> bool:
    ...
    return (
        ...
        or bool(re.search(r"(?:^|\s)পৃ(?:[.\sঃ:]|$)", stripped))
...
def strip_bengali_tts_frontmatter(text: str) -> str:
    ...
    has_source_context = any(bengali_tts_source_like_line(line) for line in non_empty_lines[:8])
    ...
    for index, line in enumerate(lines):
        ...
        source_like = bengali_tts_source_like_line(stripped) or title_page_boilerplate
    ...
    while end_index > 0:
        stripped = body_lines[end_index - 1].strip()
        if not stripped or bengali_tts_source_like_line(stripped):
            end_index -= 1
            continue
        break
...
def prepared_text_source_terms(text: str) -> list[str]:
    ...
    if re.search(r"(?:^|\s)পৃ(?:[.\sঃ:]|$)", text):
        terms.append("পৃ.")
    if re.search(r"(?m)^\s*[০-৯\d]{3,4}\s*\([^)]*পৃ[^)]*\)\s*$", text):
        terms.append("page_reference_line")
    if re.search(r"(?m)^\s*[০-৯\d]{3,4}\s*[।.!?]?\s*$", text):
        terms.append("standalone_publication_year")
    return terms
...
def prepare_bengali_tts_text(text: str) -> str:
    ...
    prepared = "\n".join(
        ...
        if not bengali_tts_source_like_line(line.strip())
    )
...
def bengali_tts_integrity_recoverable(integrity: dict, prepared_text: str) -> bool:
    blockers = [str(item) for item in (integrity.get("blocker_reasons") or [])]
    return (
        blockers == ["Body junk detected: page_number_line"]
        and bool(integrity.get("chapter_order_match"))
        and bool(integrity.get("chapter_title_match"))
        and bool(integrity.get("toc_links_valid"))
        and not integrity.get("missing_chapters")
        and not integrity.get("duplicate_chapters")
        and not prepared_text_source_terms(prepared_text)
    )
...
def sarvam_group_repair_state(repair_plan: dict | None) -> dict:
    """Describe whether a generated manifest actually used the repair path."""
    ...
    return {
        ...
        "reason": "fresh_generation_without_repair_manifest",
    }
...
def generate_sarvam_full_pilot(args, run_dir: Path, manuscript_path: Path, manuscript: str, preflight: dict) -> dict:
    ...
    model_slug = re.sub(r"[^A-Za-z0-9]+", "_", SARVAM_FULL_PILOT_MODEL).strip("_").lower()
    voice_slug = re.sub(r"[^A-Za-z0-9]+", "_", SARVAM_FULL_PILOT_VOICE).strip("_").lower()
    style_slug = re.sub(r"[^A-Za-z0-9]+", "_", SARVAM_FULL_PILOT_STYLE).strip("_").lower()
    final_audio = run_dir / f"{args.slug}_sarvam_{model_slug}_{voice_slug}_{style_slug}_final.mp3"
    ...
    report = {
        ...
        "group_repair": sarvam_group_repair_state(repair_plan),
...
def main() -> int:
    ...
    if integrity.get("status") != "PASS":
        recoverable_bengali_tts_integrity = False
        if args.language == "ben" and sarvam_requested(args):
            manuscript_path = write_clean_manuscript_if_missing(args)
            prepared = prepare_bengali_tts_text(manuscript_path.read_text(encoding="utf-8"))
            recoverable_bengali_tts_integrity = bengali_tts_integrity_recoverable(integrity, prepared)
        if recoverable_bengali_tts_integrity:
            write_json(
                run_dir / "tts_content_integrity_recovery.json",
                {
                    "status": "PASS",
                    "reason": "Bengali reader manuscript contains page-number source lines, but TTS preparation strips them before provider calls.",
                    "original_content_integrity_status": integrity.get("status"),
                    "original_blockers": integrity.get("blocker_reasons") or [],
                    "prepared_text_source_terms": prepared_text_source_terms(prepared),
                    "chapter_order_match": integrity.get("chapter_order_match"),
                    "chapter_title_match": integrity.get("chapter_title_match"),
                    "toc_links_valid": integrity.get("toc_links_valid"),
                },
            )
        else:
            return finish(
