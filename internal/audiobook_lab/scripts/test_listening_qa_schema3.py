...
def assert_listening_qa_budget_gate_blocks_when_missing() -> None:
    with temporary_env(EARNALISM_OPENAI_LISTENING_QA_MAX_ESTIMATED_USD=None, MAX_TTS_BUDGET_USD="5"):
        guard = openai_listening_qa_budget_guard(sample_count=1, total_estimated_usd=0.0119)
    assert not guard["ok"], guard
    assert guard["code"] == "LISTENING_QA_BUDGET_GATE_MISSING", guard


def assert_listening_qa_budget_gate_blocks_over_cap() -> None:
    with temporary_env(
        EARNALISM_OPENAI_LISTENING_QA_MAX_ESTIMATED_USD="0.01",
        EARNALISM_OPENAI_LISTENING_QA_ESTIMATED_USD="0.05",
        MAX_TTS_BUDGET_USD="5",
    ):
        guard = openai_listening_qa_budget_guard(sample_count=1, total_estimated_usd=0.0119)
    assert not guard["ok"], guard
    assert guard["code"] == "LISTENING_QA_BUDGET_EXCEEDED", guard


def assert_listening_qa_budget_gate_allows_within_cap() -> None:
    with temporary_env(
    ...
    assert guard["estimated_qa_cost_usd"] == 0.05, guard
    assert guard["total_estimated_usd"] == 0.0619, guard
...
class FakeCompletions:
    ...
    def create(self, **kwargs):
        self.calls += 1
        raise AssertionError("OpenAI provider call should not run before listening-QA budget gate passes")
...
def assert_openai_call_not_made_before_budget_gate() -> None:
    with tempfile.NamedTemporaryFile(suffix=".mp3") as sample_audio:
        sample_audio.write(b"not-real-audio-but-nonempty")
        sample_audio.flush()
        client = FakeOpenAIClient()
        args = type("Args", (), {"language": "Bengali", "title": "Anandamath", "author": "Bankim"})()
        sample = {
            "sample_label": "budget_gate_probe",
            "sample_audio_path": sample_audio.name,
            "sample_audio_hash": "c" * 64,
        }
        with temporary_env(EARNALISM_OPENAI_LISTENING_QA_MAX_ESTIMATED_USD=None, MAX_TTS_BUDGET_USD="5"):
            judged = judge_audio_sample_with_openai(client, args, sample)
    assert client.chat.completions.calls == 0, client.chat.completions.calls
...
def main() -> int:
    ...
    assert_listening_qa_budget_gate_blocks_when_missing()
