...
| --- | --- | --- | --- | --- | --- |
| `/` Home desktop and mobile | `frontend/src/components/ReferencePublicPages.jsx` (`ReferenceHomeSurface`) | `ReferencePublicPages.css`, `styles/quiet-heritage.css`; header excluded | `publicAccessCopy.js`; supplied still-life asset | `Home.test.jsx`, `measure_primary_visual_structure.mjs` Home states | `e398e2f68` |
| `/library` desktop and mobile | `ReferenceLibrarySurface` | `ReferencePublicPages.css`, `styles/quiet-heritage.css` | catalog API with release-safe fallback | `Library.test.jsx`, Library direct states | `78c514a74` |
...
| `/pricing` Reading Pass desktop/mobile | `ReferenceCommerceSurface` | `ReferencePublicPages.css`, `styles/quiet-heritage.css`; header excluded | configured offers; local-only offer fixture for visual measurement; `publicAccessCopy.js` | `Pricing.test.jsx`, Commerce direct states | `a8fcb9d87` |
...
| `/account` Profile mobile | `frontend/src/pages/Account.jsx` | `styles/auth-account.css`, `styles/quiet-heritage.css`, `experiences-v2.css` | existing `AuthContext` profile, Reading Pass, activity, and device fields; no fabricated saved title | `authAccountTextRules.test.js`, sanitized fixture capture | pre-existing account approval |
