...
def load_payload(path: Path | None, *, sample: bool) -> dict[str, Any]:
    if sample:
        return {**SAMPLE_BOOK, "cleaned_text": SAMPLE_TEXT, "source_hash": hash_source(SAMPLE_TEXT)}
    if path is None:
...
def write_reports(result, output_dir: Path) -> tuple[Path, Path, Path]:
...
def main() -> int:
    ...
    json_path, csv_path, md_path = write_reports(result, args.output_dir)
    print(
