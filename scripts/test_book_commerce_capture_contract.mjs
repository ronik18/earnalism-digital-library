...
import {
  ...
  CURRENT_OFFER_BUTTONS,
  resolveCaptureIdentity,
  writeCaptureFailure,
} from "./lib/book_commerce_capture_contract.mjs";

const historicalNames = ["Choose The Opening Hour", "Choose The Quiet Hour", "Choose The Deep Reading Pass", "Choose The Reader’s Reserve"];
const currentNames = CURRENT_OFFER_BUTTONS;
...
pass("historical mode records its actual no-test-id DOM instead of a candidate PASS", () => assert.equal(evaluateOfferObservation({ mode: HISTORICAL_CAPTURE_MODE, cardCount: 4, buttonNames: historicalNames, testIds: [] }).result, "HISTORICAL_OBSERVED"));
pass("current candidate mode requires every current offer test ID and direct-minute label", () => assert.equal(evaluateOfferObservation({ mode: CURRENT_CAPTURE_MODE, cardCount: 4, buttonNames: currentNames, testIds: ids }).result, "PASS"));
pass("missing candidate CTA or wrong card count still fails", () => {
  assert.equal(evaluateOfferObservation({ mode: CURRENT_CAPTURE_MODE, cardCount: 3, buttonNames: currentNames.slice(0, 3), testIds: ids.slice(0, 3) }).result, "FAIL");
});
