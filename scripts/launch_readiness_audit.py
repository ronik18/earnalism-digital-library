...
PAYMENT_SMOKE_EVENTS = ["pricing_page_view", "checkout_started", "payment_success_return", "payment_failed_or_cancelled"]
...
def run_payment_smoke() -> dict[str, Any]:
    ...
    checks = {
        ...
        "no_public_audiobook_sale_detected": "PUBLIC_AUDIO_RELEASE_BLOCKED" in server or "Audiobook experience in private review" in public_payment_copy,
...
def analytics_event_schema() -> dict[str, Any]:
    event_metadata: dict[str, list[str]] = {
        "homepage_view": ["path", "launch_status", "public_audio_status"],
        "first_time_site_tour_shown": ["source", "path", "total_steps"],
        "first_time_site_tour_completed": ["source", "step_key", "step_index", "total_steps"],
        "first_time_site_tour_skipped": ["source", "step_key", "step_index", "total_steps"],
        "hero_read_chapter_free_click": ["book_slug", "cta"],
        "dracula_book_page_view": ["book_slug"],
        "start_dracula_click": ["book_slug", "cta", "source"],
        "return_resume_reading_click": ["book_slug", "source"],
        "reader_opened": ["book_slug", "chapter_id", "is_preview", "source"],
        "reader_locked_state": ["book_slug", "reason", "source"],
        "reader_low_balance_state": ["book_slug", "wallet_seconds", "source"],
        "pricing_page_view": ["selected_pack_id", "coupon", "source"],
        "reading_pack_selected": ["source"],
        ...
        "continue_reading_click": ["book_slug", "source"],
        "core_web_vital": ["metric", "value", "rating", "path"],
    }
    blocked_fields = [
        "email",
        "phone",
...
def validate_mock_analytics_events(schema: dict[str, Any]) -> dict[str, Any]:
    ...
    mock_payloads = [
        ...
        {
            "event": "first_time_site_tour_shown",
            "metadata": {"source": "first_visit", "path": "/", "total_steps": 4},
        },
        {
            "event": "first_time_site_tour_completed",
            "metadata": {"source": "first_visit", "step_key": "shelves", "step_index": 4, "total_steps": 4},
        },
        {
            "event": "first_time_site_tour_skipped",
            "metadata": {"source": "forced_query", "step_key": "welcome", "step_index": 1, "total_steps": 4},
        },
        {"event": "hero_read_chapter_free_click", "metadata": {"book_slug": "dracula", "cta": "read_chapter_1_free"}},
        {"event": "dracula_book_page_view", "metadata": {"book_slug": "dracula"}},
        {"event": "start_dracula_click", "metadata": {"book_slug": "dracula", "cta": "start-reading", "source": "book_detail"}},
        {
            "event": "reader_opened",
            "metadata": {"book_slug": "dracula", "chapter_id": "chapter-1", "is_preview": True, "source": "reader_load"},
        },
        {
            "event": "reader_locked_state",
            "metadata": {"book_slug": "dracula", "reason": "INSUFFICIENT_READING_TIME", "source": "chapter_gate"},
        },
        {
            "event": "reader_low_balance_state",
            "metadata": {"book_slug": "dracula", "wallet_seconds": 240, "source": "reader_pulse"},
        },
        {"event": "pricing_page_view", "metadata": {"selected_pack_id": "30m", "coupon": "", "source": "pricing"}},
        {"event": "reading_pack_selected", "metadata": {"source": "pricing", "pack_id": "1h", "price_inr": 89}},
        {"event": "checkout_started", "metadata": {"pack_id": "30m", "price_inr": 49, "coupon": "", "source": "pricing", "payment_mode": "test"}},
        {"event": "payment_success_return", "metadata": {"pack_id": "30m", "price_inr": 49, "minutes": 30, "source": "test_mode_simulator", "credited": True}},
        {"event": "payment_failed_or_cancelled", "metadata": {"pack_id": "30m", "price_inr": 49, "reason": "operator_test_declined"}},
        {"event": "wallet_credited_visible", "metadata": {"pack_id": "30m", "minutes": 30, "source": "test_mode_simulator"}},
        {"event": "continue_reading_click", "metadata": {"book_slug": "dracula", "source": "pricing_continue"}},
        {"event": "return_resume_reading_click", "metadata": {"book_slug": "dracula", "source": "account_continue_reading"}},
        {"event": "core_web_vital", "metadata": {"metric": "LCP", "value": 1420.25, "rating": "good", "path": "/"}},
    ]
    errors: list[str] = []
...
def first_batch_backfill_plan() -> dict[str, Any]:
    metadata = {
        "Anandamath Visual Study Companion": ("Bankim Chandra Chattopadhyay", "Bengali"),
        "Devdas Study Edition": ("Sarat Chandra Chattopadhyay", "Bengali"),
        "Abol Tabol Illustrated Reader": ("Sukumar Ray", "Bengali"),
        "Sultana's Dream Feminist Sci-Fi Edition": ("Rokeya Sakhawat Hossain", "English"),
        "Sherlock Holmes Logic Workbook": ("Arthur Conan Doyle", "English"),
        "Dracula Gothic Fiction Visual Guide": ("Bram Stoker", "English"),
        "Frankenstein Science & Ethics Guide": ("Mary Shelley", "English"),
        "Tagore Short Stories for Young Readers": ("Rabindranath Tagore", "Bengali/English"),
        "Calculus Made Easy Visual Guide": ("Silvanus P. Thompson", "English"),
        "Chander Pahar Adventure Companion": ("Bibhutibhushan Bandyopadhyay", "Bengali"),
    }
    rows = []
    for title in FIRST_BATCH:
        ...
        author, language = metadata[title]
        rows.append(
            {
                "title": title,
                "slug": slugify(title),
                "author": author,
                "language": language,
                "current_status": "pipeline",
                "public_visibility": "not public/live",
                "source_url": "SOURCE_METADATA_REQUIRED",
                "source_name": "SOURCE_METADATA_REQUIRED",
                "source_license": "SOURCE_METADATA_REQUIRED",
                "source_license_url": "SOURCE_METADATA_REQUIRED",
                "commercial_use_status": "UNKNOWN_COMMERCIAL_USE",
                "source_hash": "SOURCE_METADATA_REQUIRED",
                "content_hash": "SOURCE_METADATA_REQUIRED",
                "provenance_hash": "SOURCE_METADATA_REQUIRED",
                "attribution_requirement": "SOURCE_METADATA_REQUIRED",
                "derivative_audiobook_rights_status": "UNKNOWN_DERIVATIVE_RIGHTS",
                "public_metadata_allowed": "no",
                "public_cta_allowed": "no",
                "owner_approval_status": "OWNER_APPROVAL_REQUIRED",
                "go_hold_decision": "HOLD_SOURCE_RIGHTS_QA_REQUIRED",
                ...
                "blocking_reason": "Real source metadata, commercial-use evidence, hashes, derivative/audio rights evidence, QA, and owner approval are not present in the dry-run batch.",
            }
        )
...
def write_first_batch_source_matrix(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in plan["rows"]:
        matrix_row = {
            ...
            "derivative_audiobook_rights_status": row["derivative_audiobook_rights_status"],
            "public_metadata_allowed": row["public_metadata_allowed"],
            "public_cta_allowed": row["public_cta_allowed"],
            "owner_approval_status": row["owner_approval_status"],
            "go_hold_decision": row["go_hold_decision"],
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "",
                "No item is ready for publication because real source URLs, source licenses, license URLs, commercial-use status, source hashes, content hashes, provenance hashes, derivative/audiobook rights status, owner approval, and final QA evidence are missing from the launch evidence.",
                "",
                markdown_table(
                    [
                        "Title",
                    ...
                    [
                        [
                            ...
                            row["public_cta_allowed"],
                            row["derivative_audiobook_rights_status"],
                            row["go_hold_decision"],
                            row["blocking_reason"],
                        ]
                        for row in rows
                    ],
