...
def audit_seo() -> dict[str, Any]:
    ...
    homepage_checks = {
        "canonical_preview_copy": "first 3 canonical pages" in index_html.lower() and "no subscription or autorenewal" in index_html.lower(),
...
def run_payment_smoke() -> dict[str, Any]:
    ...
    public_audio_block_copy_detected = (
        ...
        or "audiobook experience in private review" in lower_public_payment_copy
        or "no reader, checkout, or audiobook access" in lower_public_payment_copy
    )
    public_audiobook_sale_terms_absent = not any(
        term in lower_public_payment_copy
        for term in ["audiobook pass", "buy audiobook", "listen now", "audiobook available"]
    )
    checks = {
        "dry_run_only": True,
        ...
        "wallet_truth_copy_detected": "used only while you read" in public_payment_copy and "wallet" in lower_public_payment_copy,
        "no_subscription_autorenewal_copy_detected": "No subscription or autorenewal" in public_payment_copy,
        "no_permanent_ownership_claim_detected": not any(term in lower_public_payment_copy for term in forbidden_payment_claims),
        "no_public_audiobook_sale_detected": public_audio_block_copy_detected and public_audiobook_sale_terms_absent,
