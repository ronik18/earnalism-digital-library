...
test("existing Account-mobile state is reused", () => assert.equal(manifest.states.filter((state) => state.route === "/account" && state.viewport.width === 390 && state.viewport.height === 844 && state.fixture === "sanitized-account").map((state) => state.id).join(","), "account-mobile"));
