...
function assertPrivate(record, emptyLibrary = false) { assert.equal(record.private_fixture.fixture_visible, true); assert.equal(record.private_fixture.sensitive_fixture_values_present, false); assert.equal(record.private_fixture.production_authentication_used, false); assert.equal(record.private_fixture.production_account_api_called, false); assert.equal(record.private_fixture.unfulfilled_api_request_count, 0); assert.ok(record.private_fixture.fixture_intercepted_api_request_count >= 0); assert.equal(record.private_fixture.mutation_count, 0); assert.match(record.private_fixture.fixture_sha256, /^[a-f0-9]{64}$/); if (emptyLibrary) assert.equal(record.private_fixture.my_library_empty_state_visible, true); }
