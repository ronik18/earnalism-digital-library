...
def main() -> int:
    ...
    p.add_argument("--task-id", required=True); p.add_argument("--candidate-head", required=True)
    ...
    if actual != a.candidate_head or worker.get("tested_revision") != actual:
    ...
    result = {"task_id": a.task_id, "tested_revision": actual, "decision": decision, "findings": findings, "generated_at": int(time.time())}
