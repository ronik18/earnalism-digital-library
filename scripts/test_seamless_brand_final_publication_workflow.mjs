...
pass("keeps synthetic hash authority out of real workflow validation", !/SEAMLESS_BRAND_EXPECTED_PRODUCTION_SURFACE|synthetic-production-surface/.test(captureStep?.run || ""));
pass("runs final evidence generation only after real matrix validation", before("Capture the exact head in all browsers", "Generate and validate final evidence inputs"));
...
pass("runs the historic and current Library-baseline validators before browser evidence", before("Run deterministic tooling gates", "Run browser tooling gates") && /node scripts\/test_pr360_library_interaction_baseline\.mjs/.test(workflow) && /node scripts\/test_pr362_library_interaction_baseline\.mjs/.test(workflow) && /node scripts\/test_pr364_library_interaction_baseline\.mjs/.test(workflow) && /node scripts\/test_pr371_library_interaction_baseline\.mjs/.test(workflow) && /node scripts\/test_pr372_library_interaction_baseline\.mjs/.test(workflow) && /node scripts\/test_pr376_library_interaction_baseline\.mjs/.test(workflow) && /node scripts\/test_pr377_library_interaction_baseline\.mjs/.test(workflow) && /node scripts\/test_issue380_library_interaction_baseline\.mjs/.test(workflow) && /node scripts\/test_issue380_ui_completion_library_interaction_baseline\.mjs/.test(workflow) && /node scripts\/test_pr397_library_interaction_baseline\.mjs/.test(workflow) && /node scripts\/test_pr399_library_interaction_baseline\.mjs/.test(workflow) && /node scripts\/test_pr414_library_interaction_baseline\.mjs/.test(workflow));
...
pass("requires both full-scale Book Detail owner-review PDF headings", ['Book Detail — Dracula', 'Book Detail — Devdas (Bengali)'].every((heading) => (packageStep?.run || "").includes(heading)));
pass("writes a structured final package-QA result", /final-package-qa-results\.json/.test(packageStep?.run || "") && /PDF_required_headings/.test(packageStep?.run || "") && /contact_sheet_nonblank/.test(packageStep?.run || ""));
pass("fails package QA when the local HTML server never becomes reachable", /package_server_ready=false/.test(packageStep?.run || "") && /test "\$package_server_ready" = true/.test(packageStep?.run || ""));
pass("preserves final QA evidence in the uploaded envelope and independently verifies it", /cp "\$RUNNER_TEMP\/final-package-test-report\.json"/.test(workflow) && /cp "\$RUNNER_TEMP\/final-package-qa-results\.json"/.test(workflow) && /final-package-qa-results\.json/.test(workflow) && /pdf-tooling-preflight\.json/.test(workflow));
