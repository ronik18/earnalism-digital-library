...
async function functionalCheck(state) {
  ...
  const before = await body.evaluate((node) => Number.parseFloat(getComputedStyle(node).fontSize)); const initialText = await scale.textContent(); await increase.click(); const afterIncrease = await body.evaluate((node) => Number.parseFloat(getComputedStyle(node).fontSize)); const increasedText = await scale.textContent(); await decrease.click(); const afterDecrease = await body.evaluate((node) => Number.parseFloat(getComputedStyle(node).fontSize)); const decreasedText = await scale.textContent(); await settings.focus(); const settingsFocused = await page.evaluate(() => document.activeElement?.getAttribute("aria-label") === "Reader settings"); await back.focus(); const focusOrder = []; for (let index = 0; index < 3; index += 1) { await page.keyboard.press("Tab"); focusOrder.push(await page.evaluate(() => document.activeElement?.getAttribute("aria-label"))); }
  const decreaseDisabled = await decrease.isDisabled();
  await browser.close(); return { before, afterIncrease, afterDecrease, initialText, increasedText, decreasedText, settingsFocused, focusOrder, decreaseDisabled, errors };
}
...
test("keyboard focus order skips the disabled minimum-size control", () => [functional390, functional320, functional320High].forEach((result) => { assert.equal(result.decreaseDisabled, true); assert.deepEqual(result.focusOrder, ["Increase text size", "Reader settings", "Go to page"]); }));
