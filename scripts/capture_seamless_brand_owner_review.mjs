...
function routeSurfaceHashes() {
  return {
    ...
    shared_footer: hashFileSet(["frontend/src/components/Footer.jsx", "frontend/src/components/FooterSocialLinks.jsx", "frontend/src/components/FooterWordmark.jsx", "frontend/src/components/FooterWordmark.css", "frontend/public/assets/brand/earnalism-footer-wordmark-ivory.png", "frontend/public/assets/brand/earnalism-footer-wordmark-ivory.provenance.json"]),
...
const STABILITY_STYLE_ID = "seamless-brand-evidence-stabilization";
...
async function visualFingerprint(page) {
  return page.evaluate((styleId) => {
    ...
    const article = document.querySelector('[data-testid="journal-article"]');
    const header = document.querySelector('[data-testid="site-header"], header.experience-header, header[data-testid="status-brand-masthead"]');
    const root = article || document.body;
    const fonts = [...root.querySelectorAll("h1,p,a,span")].slice(0, 32).map((node) => {
    ...
    return {
      ...
      fonts,
      scroll: [window.scrollX, window.scrollY, window.visualViewport?.offsetLeft || 0, window.visualViewport?.offsetTop || 0],
...
async function installVisualCaptureStabilization(page, browserName) {
  ...
  if (browserName !== "webkit" || !atTop) return undefined;
  return page.addStyleTag({
    content: `#${STABILITY_STYLE_ID}{display:block!important}*{-webkit-font-smoothing:antialiased!important}[data-testid="site-header"],header.experience-header,header[data-testid="status-brand-masthead"]{position:static!important}`,
  });
}
...
async function waitForVisualQuiescence(page, state, fixtureRequests) {
  if (!state.id.startsWith("article-")) return { result: "PASS", applicable: false };
  const expectedTitle = editorialFixture().post.title;
  const expectedRequests = [`/api/blog/${editorialFixture().post.slug}`, "/api/blog"];
  const deadline = Date.now() + 10000;
  while (expectedRequests.some((entry) => !fixtureRequests.has(entry))) {
    if (Date.now() > deadline) throw new Error(`State ${state.id}: editorial fixture requests did not complete: ${expectedRequests.filter((entry) => !fixtureRequests.has(entry)).join(", ")}.`);
    await page.waitForTimeout(25);
  }
  const result = await page.evaluate(async ({ expectedTitle, quietMs }) => {
    ...
    const root = document.querySelector('[data-testid="journal-article"]');
    const heading = root?.querySelector("h1");
    if (document.querySelector('[data-testid="journal-article-loading"]') || !visible(root) || !visible(heading) || heading.textContent?.trim() !== expectedTitle) throw new Error("article hydration preconditions are not satisfied");
    const imageNodes = [...root.querySelectorAll("img")].filter(visible);
    await Promise.all(imageNodes.map((image) => image.decode().catch(() => undefined)));
    const textNodes = [heading, root.querySelector('[data-testid="back-journal"]'), ...root.querySelectorAll("p,span,a")].filter(visible);
    const fontSpecifications = [...new Set(textNodes.map((node) => { const style = getComputedStyle(node); return `${style.fontStyle} ${style.fontWeight} ${style.fontSize} ${style.fontFamily}`; }))];
    await document.fonts.ready;
    for (const spec of fontSpecifications) { await document.fonts.load(spec, "Aa"); if (!document.fonts.check(spec, "Aa")) throw new Error(`font is not ready: ${spec}`); }
    const geometry = () => [document.documentElement.scrollWidth, document.documentElement.scrollHeight, ...[root, heading, root.querySelector('[data-testid="back-journal"]'), root.querySelector("header"), root.querySelector(".editorial-prose"), root.querySelector('[data-testid="article-share"]')].map((node) => { const r = node?.getBoundingClientRect(); return r ? [r.left, r.top, r.right, r.bottom, r.width, r.height].map((v) => Math.round(v * 100) / 100) : null; }), window.scrollX, window.scrollY, window.visualViewport?.offsetLeft || 0, window.visualViewport?.offsetTop || 0];
    let lastMutation = performance.now(); const mutations = [];
    const observer = new MutationObserver((records) => { lastMutation = performance.now(); mutations.push(...records.map((record) => ({ type: record.type, attribute: record.attributeName || null, added: record.addedNodes.length, removed: record.removedNodes.length }))); });
    observer.observe(root, { childList: true, subtree: true, characterData: true, attributes: true });
    const samples = [];
    try {
      while (samples.length < 4 || performance.now() - lastMutation < quietMs) {
        await new Promise((resolve) => setTimeout(resolve, 150));
        samples.push(geometry()); if (samples.length > 4) samples.shift();
        if (samples.length === 4 && new Set(samples.map((sample) => JSON.stringify(sample))).size === 1 && performance.now() - lastMutation >= quietMs) break;
      }
      if (new Set(samples.map((sample) => JSON.stringify(sample))).size !== 1) throw new Error("article geometry did not settle");
      await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
      return { result: "PASS", applicable: true, expected_title: expectedTitle, related_section: root.querySelector("section") ? "VISIBLE" : "ABSENT_BY_FIXTURE", font_specifications: fontSpecifications, mutation_count_during_quiescence: mutations.length, quiet_window_ms: quietMs, geometry_samples: samples, layout_fingerprint: JSON.stringify(samples.at(-1)), font_fingerprint: JSON.stringify(fontSpecifications), scroll_fingerprint: JSON.stringify(samples.at(-1).slice(-4)) };
    } finally { observer.disconnect(); }
  }, { expectedTitle, quietMs: 750 });
  return result;
}
...
async function libraryFilterInteraction(page, state) {
  ...
  result.geometry = await page.evaluate(() => {
    const box = (node) => { const rect = node?.getBoundingClientRect(); return rect ? { left: rect.left, top: rect.top, right: rect.right, bottom: rect.bottom, width: rect.width, height: rect.height } : null; };
    const visible = (node) => { const style = node && getComputedStyle(node); const rect = node?.getBoundingClientRect(); return Boolean(style && rect && style.display !== "none" && style.visibility !== "hidden" && rect.width > 0 && rect.height > 0); };
    const trigger = [...document.querySelectorAll('button.reference-filter-trigger')].find(visible); const panel = [...document.querySelectorAll('.reference-library-drawer[role="dialog"][aria-modal="true"]')].find(visible); const content = panel?.firstElementChild; const apply = [...panel?.querySelectorAll("button") || []].find((node) => /(?:apply filters|show\s+\d+\s+editions?)/i.test(node.textContent || "")); const close = panel?.querySelector('button[aria-label="Close filters"]'); const style = panel && getComputedStyle(panel);
  ...
  // The current paper Library applies facets as selected; its primary drawer
  // action truthfully returns with the live result count rather than implying
  // a deferred, separate apply step.
...
async function footerScrollInteraction(page, state) {
  ...
  const wordmark = footer.locator('[data-testid="footer-wordmark"]:visible');
  const wordmarkImage = wordmark.locator('img:visible');
  const result = { kind: "scroll-to-footer", footer_count: await footer.count(), footer_wordmark_count: await wordmark.count(), footer_wordmark_image_count: await wordmarkImage.count(), footer_in_view: false, navigation_reachable: false, legal_links_reachable: false, geometry: undefined, failures: [] };
  if (result.footer_count !== 1 || result.footer_wordmark_count !== 1 || result.footer_wordmark_image_count !== 1) {
    result.failures.push("footer-brand-surface");
    return { result, capture_surface: wordmark, capture_lockup: wordmarkImage, finalize: async () => result };
  }
  ...
  result.geometry = await page.evaluate(() => {
    ...
    const footer = document.querySelector('[data-testid="site-footer"]'); const wordmark = footer?.querySelector('[data-testid="footer-wordmark"]'); const image = wordmark?.querySelector("img"); const navigation = [...(footer?.querySelectorAll("nav a") || [])]; const legal = [...(footer?.querySelectorAll("a") || [])]; const logo = box(wordmark); const navBoxes = navigation.map(box).filter(Boolean); const legalBoxes = legal.map(box).filter(Boolean); const wordmarkStyle = wordmark && getComputedStyle(wordmark); const imageStyle = image && getComputedStyle(image);
    const clipped = (rect) => Boolean(rect && (rect.left < 0 || rect.right > innerWidth));
    return { viewport: { width: innerWidth, height: innerHeight }, footer: box(footer), wordmark: box(wordmark), logo, navigation: navBoxes, legal_links: legalBoxes, logo_navigation_overlap_area: navBoxes.reduce((sum, item) => sum + area(logo, item), 0), clipped_control_count: [...navBoxes, ...legalBoxes].filter(clipped).length, horizontal_overflow: document.documentElement.scrollWidth > document.documentElement.clientWidth, footer_in_view: Boolean(logo && logo.bottom > 0 && logo.top < innerHeight), navigation_reachable: navBoxes.length > 0 && navBoxes.every((item) => item.width > 0 && item.height > 0 && !clipped(item)), legal_links_reachable: legalBoxes.length > 0 && legalBoxes.every((item) => item.width > 0 && item.height > 0 && !clipped(item)), wordmark_style: wordmark ? { background: wordmarkStyle.backgroundColor, border_width: wordmarkStyle.borderWidth, box_shadow: wordmarkStyle.boxShadow, padding: wordmarkStyle.padding, transform: imageStyle?.transform || "", image_source: image?.getAttribute("src") || "" } : null };
  });
  ...
  if (!result.footer_in_view || !result.navigation_reachable || !result.legal_links_reachable || result.geometry.logo_navigation_overlap_area !== 0 || result.geometry.clipped_control_count !== 0 || result.geometry.horizontal_overflow || result.geometry.wordmark_style?.background !== "rgba(0, 0, 0, 0)" || result.geometry.wordmark_style?.border_width !== "0px" || result.geometry.wordmark_style?.box_shadow !== "none" || result.geometry.wordmark_style?.padding !== "0px" || result.geometry.wordmark_style?.transform !== "none" || !result.geometry.wordmark_style?.image_source.endsWith("/assets/brand/earnalism-footer-wordmark-ivory.png")) result.failures.push("footer-zoom-contract");
  return { result, capture_surface: wordmark, capture_lockup: wordmarkImage, finalize: async () => { await page.evaluate(() => window.scrollTo(0, 0)); return result; } };
}
...
async function captureManifestState(browser, browserName, state, baseUrl, outputDirectory, contextIndex, routeInventory) {
  ...
  page.on("request", (request) => {
    ...
    if (pathname.includes("/api/")) apiRequests.push({ url: request.url(), method: request.method() });
    if (pathname === "/api/blog" || pathname === `/api/blog/${editorialFixture().post.slug}`) editorialRequestTimeline.push({ event: "start", pathname, milliseconds_since_navigation: Date.now() - navigationStartedAt });
  });
  ...
  const microStoryCampaign = await page.evaluate(captureMicroStoryCampaign, MICRO_STORY_PRIMARY_CTA_SELECTOR);
  const data = await page.evaluate(({ statusFixture, microStoryCampaign }) => {
    const visible = (node) => { if (!node) return false; const style = getComputedStyle(node); const rect = node.getBoundingClientRect(); return style.display !== "none" && style.visibility !== "hidden" && rect.width > 0 && rect.height > 0; };
