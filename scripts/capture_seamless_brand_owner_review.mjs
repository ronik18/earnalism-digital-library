...
async function footerScrollInteraction(page, state) {
  ...
  const brandPanel = footer.locator('[data-testid="footer-brand-lockup"]:visible');
  const lockup = brandPanel.locator('[data-testid="earnalism-brand-lockup"]:visible');
  const result = { kind: "scroll-to-footer", footer_count: await footer.count(), footer_brand_panel_count: await brandPanel.count(), footer_lockup_count: await lockup.count(), footer_in_view: false, navigation_reachable: false, legal_links_reachable: false, geometry: undefined, failures: [] };
  if (result.footer_count !== 1 || result.footer_brand_panel_count !== 1 || result.footer_lockup_count !== 1) {
    result.failures.push("footer-brand-surface");
    return { result, capture_surface: brandPanel, capture_lockup: lockup, finalize: async () => result };
  }
  await brandPanel.evaluate((node) => {
  ...
  result.geometry = await page.evaluate(() => {
    ...
    const footer = document.querySelector('[data-testid="site-footer"]'); const panel = footer?.querySelector('[data-testid="footer-brand-lockup"]'); const lockup = panel?.querySelector('[data-testid="earnalism-brand-lockup"]'); const image = lockup?.querySelector("img"); const navigation = [...(footer?.querySelectorAll("nav a") || [])]; const legal = [...(footer?.querySelectorAll("a") || [])]; const logo = box(lockup); const navBoxes = navigation.map(box).filter(Boolean); const legalBoxes = legal.map(box).filter(Boolean); const wrapper = lockup && getComputedStyle(lockup); const panelStyle = panel && getComputedStyle(panel);
    const clipped = (rect) => Boolean(rect && (rect.left < 0 || rect.right > innerWidth));
    return { viewport: { width: innerWidth, height: innerHeight }, footer: box(footer), brand_panel: box(panel), logo, navigation: navBoxes, legal_links: legalBoxes, logo_navigation_overlap_area: navBoxes.reduce((sum, item) => sum + area(logo, item), 0), clipped_control_count: [...navBoxes, ...legalBoxes].filter(clipped).length, horizontal_overflow: document.documentElement.scrollWidth > document.documentElement.clientWidth, footer_in_view: Boolean(logo && logo.bottom > 0 && logo.top < innerHeight), navigation_reachable: navBoxes.length > 0 && navBoxes.every((item) => item.width > 0 && item.height > 0 && !clipped(item)), legal_links_reachable: legalBoxes.length > 0 && legalBoxes.every((item) => item.width > 0 && item.height > 0 && !clipped(item)), panel: panel ? { background: panelStyle.backgroundColor, border_width: panelStyle.borderWidth, border_radius: panelStyle.borderRadius, box_shadow: panelStyle.boxShadow } : null, wrapper: lockup ? { background: wrapper.backgroundColor, border_width: wrapper.borderWidth, border_radius: wrapper.borderRadius, box_shadow: wrapper.boxShadow, padding: wrapper.padding, transform: image ? getComputedStyle(image).transform : "" } : null };
  });
  ...
  if (!result.footer_in_view || !result.navigation_reachable || !result.legal_links_reachable || result.geometry.logo_navigation_overlap_area !== 0 || result.geometry.clipped_control_count !== 0 || result.geometry.horizontal_overflow || result.geometry.panel?.background !== "rgb(255, 249, 238)" || result.geometry.panel?.border_width !== "0px" || result.geometry.panel?.border_radius !== "0px" || result.geometry.panel?.box_shadow !== "none" || result.geometry.wrapper?.background !== "rgba(0, 0, 0, 0)" || result.geometry.wrapper?.border_width !== "0px" || result.geometry.wrapper?.border_radius !== "0px" || result.geometry.wrapper?.box_shadow !== "none" || result.geometry.wrapper?.padding !== "0px" || result.geometry.wrapper?.transform !== "none") result.failures.push("footer-zoom-contract");
  return { result, capture_surface: brandPanel, capture_lockup: lockup, finalize: async () => { await page.evaluate(() => window.scrollTo(0, 0)); return result; } };
}
