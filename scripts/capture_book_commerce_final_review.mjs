...
const captureMode = process.env.BOOK_COMMERCE_CAPTURE_MODE || CURRENT_CAPTURE_MODE;
const checkout = process.env.ACTUAL_CHECKOUT_SHA || execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim();
const tree = process.env.CHECKOUT_TREE_SHA || execFileSync("git", ["rev-parse", "HEAD^{tree}"], { encoding: "utf8" }).trim();
const captureIdentity = resolveCaptureIdentity({ mode: captureMode, baseline: process.env.BOOK_COMMERCE_BASELINE || "", prHead: process.env.PR_HEAD_SHA || checkout, checkout, tree });
...
async function routes(page, { rejectOffers = false } = {}) {
  await page.route("**/api/**", async (route) => {
    ...
    // The real checkout considers its test mode selectable. Keep the fixture
    // semantically equivalent so the review exercises its ordinary unauthenticated
    // login handoff rather than an artificially unavailable offer state.
    if (p.endsWith("/payments/offers")) return rejectOffers ? route.fulfill({ status: 503, contentType: "application/json", body: JSON.stringify({ detail: "fixture offers unavailable" }) }) : json(route, { packs, config: { mode: "test", recurring_enabled: false } });
    if (p.endsWith("/payments/packs")) return json(route, packs);
    if (p.endsWith("/payments/config")) return json(route, { mode: "test", recurring_enabled: false });
...
async function verifyOfferContract(context, rejectOffers) {
  const page = await context.newPage();
  await page.setViewportSize({ width: 1440, height: 1000 });
  await routes(page, { rejectOffers });
  await page.goto(`${baseUrl}/pricing`, { waitUntil: "domcontentloaded" });
  if (!publicPaidCommerceEnabled) {
    await page.getByTestId("paid-commerce-disabled").waitFor();
    const offerCards = await page.locator("[data-testid^=pricing-pack-]").count();
    const result = {
      mode: captureMode,
      configured_public_paid_commerce: false,
      rendered_offer_count: offerCards,
      endpoint: "NOT_CALLED — paid commerce is disabled before offer retrieval",
      result: offerCards === 0 ? "NOT_APPLICABLE_PAID_COMMERCE_DISABLED" : "FAIL",
    };
    await page.close();
    if (result.result === "FAIL") throw new Error(`Disabled-commerce contract failed: ${JSON.stringify(result)}`);
    return result;
  }
  const historical = captureMode === HISTORICAL_CAPTURE_MODE;
  const offerCardSelector = historical
    ? ".reference-commerce__packs .reference-offer"
    : "[data-testid=pricing-reference-surface] .rp-offer";
  await (historical ? page.locator(offerCardSelector).first() : page.getByTestId("pricing-pack-30m")).waitFor();
  const cards = await page.locator(offerCardSelector).count();
  const buttonNames = (await page.locator(`${offerCardSelector} button`).allTextContents()).map((value) => value.trim());
  const testIds = await page.locator('[data-testid^="pricing-pack-"]').evaluateAll((nodes) => nodes.map((node) => node.getAttribute("data-testid")));
  await page.close();
  const observation = evaluateOfferObservation({ mode: captureMode, cardCount: cards, buttonNames, testIds });
  const contract = { endpoint: rejectOffers ? "/payments/packs + /payments/config fallback" : "/payments/offers", ...observation };
  if (contract.result === "FAIL") throw new Error(`Offer contract failed in ${captureMode}: ${JSON.stringify(contract)}`);
  return contract;
}
...
async function probeFonts(page) {
  return page.evaluate(async () => {
    const faces = [["cormorant", '500 32px "Cormorant Garamond"', "Earnalism"], ["outfit", '400 16px "Outfit"', "Earnalism"], ["notoSerifBengali", '500 32px "Noto Serif Bengali"', "বাংলা"], ["notoSansBengali", '400 16px "Noto Sans Bengali"', "বাংলা"]];
    const probe = document.createElement("div"); probe.setAttribute("aria-hidden", "true"); probe.style.cssText = "position:fixed;left:-10000px;top:-10000px;width:1px;height:1px;overflow:hidden;opacity:0;pointer-events:none;";
    for (const [id, shorthand, glyphs] of faces) { const span=document.createElement("span"); span.dataset.fontProbe=id; span.style.font=shorthand; span.textContent=glyphs; probe.append(span); }
    document.body.append(probe); const result={};
    for (const [id, shorthand, glyphs] of faces) { await document.fonts.load(shorthand,glyphs); const span=probe.querySelector(`[data-font-probe="${id}"]`); const style=getComputedStyle(span); const family=shorthand.match(/"([^"]+)"/)[1]; const count=[...document.fonts].filter((face)=>face.family.replaceAll('"','')===family&&face.status==="loaded").length; result[id]={requested:shorthand,glyphs,loaded_face_count:count,check:document.fonts.check(shorthand,glyphs),computed_family:style.fontFamily,computed_weight:style.fontWeight,status:count>0&&document.fonts.check(shorthand,glyphs)?"PASS":"FAIL"}; }
    probe.remove(); return result;
  });
}
function redact(headers) { return Object.fromEntries(Object.entries(headers).map(([key,value]) => /authorization|cookie|token/i.test(key) ? [key,"REDACTED"] : [key,value])); }
...
async function capture(state, context) {
  ...
  const errors = [], failedRequests = [], diagnostics = [], fontResponses = [], interceptedAssets = [];
  page.on("pageerror", (e) => errors.push(`pageerror:${e.message}`));
  page.on("console", (m) => { if (m.type() === "error") { const location=m.location(); errors.push(`console:${m.text()}`); diagnostics.push({ browser:browserName, state_id:state.id, console_error:m.text(), console_location:location.url ? { url:location.url, line:location.lineNumber, column:location.columnNumber } : null }); } });
  page.on("request", (request) => { if (request.url() === "https://theearnalism.com/assets/brand/earnalism-brand-lockup.png") interceptedAssets.push({ url: request.url(), resource_type: request.resourceType(), remediation: "fulfilled from the loopback canonical public asset" }); });
  ...
  const result = await page.evaluate(() => {
    ...
    const rows = [...document.querySelectorAll("[data-testid=chapter-list] li")]; const currentCandidate = document.querySelector("[data-testid=pricing-reference-surface]"); const grid = document.querySelector(currentCandidate ? ".rp-offer-grid" : ".reference-commerce__packs"); const cards = [...document.querySelectorAll(currentCandidate ? "[data-testid=pricing-reference-surface] .rp-offer" : ".reference-commerce__packs .reference-offer")].map((n) => ({ ...(() => { const r=n.getBoundingClientRect(); return { x:r.x,y:r.y,width:r.width,height:r.height }; })(), cta: (() => { const r=n.querySelector("button")?.getBoundingClientRect(); return r && { x:r.x,y:r.y,width:r.width,height:r.height }; })() }));
...
try { for (const state of states) { const context=await newContext(); captures.push(await capture(state,context)); await context.close(); } if (process.env.BOOK_COMMERCE_FORCE_CAPTURE_FAILURE === "after-state-matrix") throw new Error("forced capture failure after state matrix"); captureStage="interaction"; const interactionContext=await newContext(); const interactions=baseline ? { status:"NOT_APPLICABLE_HISTORICAL_SOURCE", mode:"HISTORICAL_BASELINE_CAPTURE_ONLY", reason:"Reviewed historical source predates the current tab control; this candidate-only check was not executed." } : await interaction(interactionContext); await interactionContext.close(); captureStage="offer-contract-primary"; const primaryOfferContext=await newContext(); const primaryOfferContract=await verifyOfferContract(primaryOfferContext,false); await primaryOfferContext.close(); captureStage="offer-contract-fallback"; const fallbackOfferContext=await newContext(); const fallbackOfferContract=await verifyOfferContract(fallbackOfferContext,true); await fallbackOfferContext.close(); const fixture={ source_paths:sourcePaths, sha256:sourceHash, dracula_chapter_count:draculaChapters.length, secondary_book_slug:"devdas" }; const offers={ source_path:import.meta.url, sha256:sha(Buffer.from(JSON.stringify(packs))), offer_count:packs.length, backend_packout_omits_validity_days:true, offers:packs.map((p,index)=>({order:index+1,id:p.id,label:p.label,minutes:p.minutes,price_inr:p.price_inr,amount_paise:p.amount_paise,recommended:p.recommended ?? false})), endpoint_contracts:[primaryOfferContract,fallbackOfferContract] }; const commerce=captures.filter((c)=>c.family==="commerce").map((c)=>({id:c.id, width:c.viewport.width, columns:c.commerce?.columns, cards:c.commerce?.cards||[], requirement:baseline?"NOT_APPLICABLE_HISTORICAL_SOURCE":"CURRENT_CANDIDATE_GEOMETRY", result:baseline?"HISTORICAL_OBSERVED":(c.scroll_width===c.client_width && (c.viewport.width>=1280 ? (c.commerce?.cards.length===4 && c.commerce.cards.every((x)=>x.y===c.commerce.cards[0].y)) : c.commerce?.cards.length===4)?"PASS":"FAIL")}));
 if (!publicPaidCommerceEnabled) {
  offers.current_launch_status="NOT_APPLICABLE_PAID_COMMERCE_DISABLED"; offers.offer_count=0; offers.offers=[];
  for (const record of commerce) {
   const disabled=record.cards.length===0;
   record.requirement="PAID_COMMERCE_DISABLED_ASSERTION";
   record.result=disabled ? "PASS" : "FAIL";
  }
 }
 ...
 const heading={home:{text:homeHeading,expected:expectedHomeHeading,pass:homeHeading===expectedHomeHeading},commerce:{text:null,expected:expectedCommerceHeading,pass:false}};
 await home.goto(`${baseUrl}/pricing`,{waitUntil:"domcontentloaded"}); await settle(home);
 const commerceHeading=(await home.locator(baseline ? "#reference-commerce-title" : publicPaidCommerceEnabled ? "#rp-title" : "[data-testid=paid-commerce-disabled] h1").innerText()).replace(/\s+/g," ").trim();
 heading.commerce={text:commerceHeading,expected:expectedCommerceHeading,pass:commerceHeading===expectedCommerceHeading}; await home.close(); await headingContext.close();
 captureStage="write-evidence"; const report={schema_version:"pr341-book-commerce-focused-v4",provenance:{pr_head_sha:process.env.PR_HEAD_SHA||checkout,actual_checkout_sha:checkout,tree_sha:tree,workflow_event_sha:process.env.WORKFLOW_EVENT_SHA||null,capture_mode:captureIdentity.mode,application_source_sha:captureIdentity.application_source_sha,application_tree_sha:captureIdentity.application_tree_sha,application_provenance:captureIdentity.application_provenance,capture_script_sha256:sha(fs.readFileSync(new URL(import.meta.url))),browser:browserName,browser_version:browser.version(),fixture_sha256:sha(Buffer.from(JSON.stringify({books,packs})))},book_fixture:fixture,states:captures}; const fonts={browser:browserName,version:browser.version(),faces:Object.fromEntries(["cormorant","outfit","notoSerifBengali","notoSansBengali"].map((face)=>[face,{status:captures.every((state)=>state.font_results?.[face]?.status==="PASS")?"PASS":"FAIL",states:captures.map((state)=>({id:state.id,...state.font_results?.[face],source_requests:state.font_responses}))}]))}; const stability={browser:browserName,status:captures.every((state)=>state.stable)?"PASS":"FAIL",states:captures.map((state)=>({id:state.id,status:state.stable?"PASS":"FAIL",attempts:state.stability_attempts,hashes:state.stable_screenshot_sha256}))}; const diagnostics={browser:browserName,status:captures.some((state)=>state.network_diagnostics.length)?"FAIL":"PASS",root_cause:browserName==="webkit"?{url:"https://theearnalism.com/assets/brand/earnalism-brand-lockup.png",classification:"STATIC_ASSET_403",remediation:"intercept the exact canonical-logo request and fulfill it from the loopback public asset"}:null,entries:captures.flatMap((state)=>state.network_diagnostics),intercepted_assets:captures.flatMap((state)=>state.intercepted_assets)}; fs.writeFileSync(path.join(output,"capture-results.json"),JSON.stringify(report,null,2)+"\n"); fs.writeFileSync(path.join(output,"book-interaction-results.json"),JSON.stringify(interactions,null,2)+"\n"); fs.writeFileSync(path.join(output,"fixture-offer-results.json"),JSON.stringify(offers,null,2)+"\n"); fs.writeFileSync(path.join(output,"commerce-geometry-results.json"),JSON.stringify({status:baseline?"NOT_APPLICABLE_HISTORICAL_SOURCE":commerce.every((x)=>x.result==="PASS")?"PASS":"FAIL",states:commerce},null,2)+"\n"); fs.writeFileSync(path.join(output,"heading-results.json"),JSON.stringify({status:baseline?"NOT_APPLICABLE_HISTORICAL_SOURCE":heading.home.pass&&heading.commerce.pass?"PASS":"FAIL",...heading},null,2)+"\n"); fs.writeFileSync(path.join(output,"book-content-capability-results.json"),JSON.stringify({status:"PASS",related_titles:"RELATED_TITLE_SHELF_PRODUCTION_DATA_UNAVAILABLE",library_exploration_action:"PASS",dracula:{benefits:"ABSENT",who_for:"ABSENT",learnings:"ABSENT",about_author:"ABSENT"},devdas:{benefits:"ABSENT",who_for:"ABSENT",learnings:"ABSENT",about_author:"ABSENT"}},null,2)+"\n"); fs.writeFileSync(path.join(output,"browser-font-results.json"),JSON.stringify(fonts,null,2)+"\n"); fs.writeFileSync(path.join(output,"capture-stability-results.json"),JSON.stringify(stability,null,2)+"\n"); fs.writeFileSync(path.join(output,"webkit-network-diagnostics.json"),JSON.stringify(diagnostics,null,2)+"\n"); console.log(JSON.stringify({states:captures.length,interaction:interactions.status,headings:heading.home.pass&&heading.commerce.pass,output}));
} catch (error) { writeCaptureFailure(fs, output, { capture_mode:captureIdentity.mode, application_source_sha:captureIdentity.application_source_sha, application_tree_sha:captureIdentity.application_tree_sha, stage:captureStage, error:{name:error?.name||"Error",message:error?.message||String(error)}, partial_state_ids:captures.map((state)=>state.id) }); throw error;
