...
function usage() {
  return `
Usage:
  node scripts/audio/tts/azureEnglishTts.js --input chunk.ssml --output chunk.mp3
...
async function synthesizeOnce({ inputPath, outputPath, key, region, locale, voice, outputFormat }) {
  ...
  try {
    response = await fetch(endpoint, {
      ...
      headers: {
        ...
        "User-Agent": "EarnalismEnglishAudiobookPolish",
      },
      body: ssml,
    });
  } finally {
  ...
  if (!response.ok) {
    ...
    const error = new Error(
      `Azure English TTS failed: HTTP ${response.status} ${detail}; voice=${voice}; region=${region}; retry_after=${retryAfter || "not-provided"}; ssml_preview=${ssmlPreview(ssml)}`,
    );
  ...
  if (!body.length) throw new Error("Azure English TTS returned an empty audio response");
...
async function synthesize({ inputPath, outputPath, key, region, locale, voice, outputFormat }) {
  ...
  throw lastError || new Error("Azure English TTS failed before any region was attempted");
}
...
async function main() {
  ...
  const locale = args.locale || envValue("AZURE_ENGLISH_LOCALE", "ENGLISH_TTS_LOCALE") || DEFAULT_LOCALE;
  const voice = args.voice || envValue("AZURE_ENGLISH_VOICE", "ENGLISH_TTS_VOICE_ID") || DEFAULT_VOICE;
  const outputFormat = args.outputFormat || envValue("AZURE_SPEECH_OUTPUT_FORMAT", "ENGLISH_TTS_AZURE_OUTPUT_FORMAT") || DEFAULT_OUTPUT_FORMAT;
  ...
  if (missing.length) throw new Error(`Missing required Azure English TTS config: ${missing.join(", ")}`);
