...
const DEFAULT_OUTPUT_ROOT = path.join(ROOT, "output", "english_audiobook_polish");
const BENGALI_RE = /[\u0980-\u09ff]/;
...
const REPORT_FILE = "english_audiobook_polish_report.json";
...
const DEFAULT_TTS_RATE = "+4%";
const DEFAULT_TTS_PITCH = "+1Hz";
const DEFAULT_PAUSE_PROFILE = "english-natural-expressive";
...
const PROSODY_PROFILES = {
  ...
  dialogue: { rate: "+6%", pitch: "+1Hz" },
  dramatic: { rate: "+1%", pitch: "+0Hz" },
  chapter: { rate: "-1%", pitch: "+0Hz" },
};
...
function usage() {
  return `
Usage:
  node scripts/audio/polishEnglishAudiobooks.js --sample 3
  node scripts/audio/polishEnglishAudiobooks.js --slug book-slug
  node scripts/audio/polishEnglishAudiobooks.js --all-english --commit

Selectors:
  --sample N                 Randomly audit N English audiobooks
  --slug SLUG                Process one slug; repeatable
  --all-english              Process all detected English audiobooks
  --manifest PATH            Process slugs from a queue/list JSON in file order
  --export-queue PATH        Write a English audiobook queue JSON and exit
...
Quality/TTS:
  ...
  --tts-provider NAME        Default ENGLISH_TTS_PROVIDER or "none"
  ...
  --reuse-mixed-provider     Reuse cached chunks from a different TTS provider/voice
`;
}
...
function parseArgs(argv) {
  const args = {
    ...
    resume: true,
    restart: false,
    qaThreshold: Number(process.env.ENGLISH_AUDIO_QA_THRESHOLD || DEFAULT_QA_THRESHOLD),
    ttsProvider: process.env.ENGLISH_TTS_PROVIDER || "none",
    chunkChars: Number(process.env.ENGLISH_TTS_CHUNK_CHARS || DEFAULT_CHUNK_CHARS),
    lexicon: process.env.ENGLISH_AUDIO_PRONUNCIATION_LEXICON || "",
    humanReviewed: false,
    allowReviewCommit: false,
    reuseMixedProvider: false,
    forceRegenerate: false,
    verbose: false,
    selfTestNormalization: false,
    ttsRate: process.env.ENGLISH_TTS_RATE || process.env.ENGLISH_TTS_SSML_RATE || DEFAULT_TTS_RATE,
    ttsPitch: process.env.ENGLISH_TTS_PITCH || process.env.ENGLISH_TTS_SSML_PITCH || DEFAULT_TTS_PITCH,
    ttsRequestDelayMs: Number(process.env.ENGLISH_TTS_REQUEST_DELAY_MS || process.env.ENGLISH_TTS_CHUNK_DELAY_MS || DEFAULT_TTS_REQUEST_DELAY_MS),
    ttsMaxRetries: Number(process.env.ENGLISH_TTS_MAX_RETRIES || DEFAULT_TTS_MAX_RETRIES),
    ttsRetryBaseDelayMs: Number(process.env.ENGLISH_TTS_RETRY_BASE_DELAY_MS || DEFAULT_TTS_RETRY_BASE_DELAY_MS),
    ttsRetryMaxDelayMs: Number(process.env.ENGLISH_TTS_RETRY_MAX_DELAY_MS || DEFAULT_TTS_RETRY_MAX_DELAY_MS),
    ttsRetryJitter: !/^false|0|no$/i.test(String(process.env.ENGLISH_TTS_RETRY_JITTER || "true")),
    stopOnQuota: !/^false|0|no$/i.test(String(process.env.ENGLISH_TTS_PAUSE_ON_429 || process.env.ENGLISH_TTS_STOP_ON_QUOTA || "true")),
    pauseProfile: process.env.ENGLISH_TTS_PAUSE_PROFILE || DEFAULT_PAUSE_PROFILE,
    storageFolder: process.env.ENGLISH_AUDIO_POLISH_STORAGE_FOLDER || DEFAULT_STORAGE_FOLDER,
    explicit: {},
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    } else if (token === "--all-english") {
      args.allEnglish = true;
    ...
    } else if (token === "--reuse-mixed-provider") {
      args.reuseMixedProvider = true;
...
function applyEnvDefaults(args) {
  ...
  if (!args.explicit.qaThreshold && process.env.ENGLISH_AUDIO_QA_THRESHOLD) {
    args.qaThreshold = Number(process.env.ENGLISH_AUDIO_QA_THRESHOLD) || args.qaThreshold;
  }
  if (!args.explicit.ttsProvider && process.env.ENGLISH_TTS_PROVIDER) {
    args.ttsProvider = process.env.ENGLISH_TTS_PROVIDER.trim().toLowerCase();
  }
  if (!args.explicit.chunkChars && process.env.ENGLISH_TTS_CHUNK_CHARS) {
    args.chunkChars = Math.max(300, Number.parseInt(process.env.ENGLISH_TTS_CHUNK_CHARS, 10) || args.chunkChars);
  }
  if (!args.explicit.lexicon && process.env.ENGLISH_AUDIO_PRONUNCIATION_LEXICON) {
    args.lexicon = process.env.ENGLISH_AUDIO_PRONUNCIATION_LEXICON;
  }
  if (process.env.ENGLISH_TTS_RATE || process.env.ENGLISH_TTS_SSML_RATE) {
    args.ttsRate = process.env.ENGLISH_TTS_RATE || process.env.ENGLISH_TTS_SSML_RATE;
  }
  if (process.env.ENGLISH_TTS_PITCH || process.env.ENGLISH_TTS_SSML_PITCH) {
    args.ttsPitch = process.env.ENGLISH_TTS_PITCH || process.env.ENGLISH_TTS_SSML_PITCH;
  }
  if (process.env.ENGLISH_TTS_PAUSE_PROFILE) {
    args.pauseProfile = process.env.ENGLISH_TTS_PAUSE_PROFILE;
  }
  if (process.env.ENGLISH_TTS_CONCURRENCY) {
    args.concurrency = Math.max(1, Number.parseInt(process.env.ENGLISH_TTS_CONCURRENCY, 10) || args.concurrency);
  }
  if (process.env.ENGLISH_TTS_REQUEST_DELAY_MS || process.env.ENGLISH_TTS_CHUNK_DELAY_MS) {
    args.ttsRequestDelayMs = Number(process.env.ENGLISH_TTS_REQUEST_DELAY_MS || process.env.ENGLISH_TTS_CHUNK_DELAY_MS) || args.ttsRequestDelayMs;
  }
  if (process.env.ENGLISH_TTS_MAX_RETRIES) {
    args.ttsMaxRetries = Number(process.env.ENGLISH_TTS_MAX_RETRIES) || args.ttsMaxRetries;
  }
  if (process.env.ENGLISH_TTS_RETRY_BASE_DELAY_MS) {
    args.ttsRetryBaseDelayMs = Number(process.env.ENGLISH_TTS_RETRY_BASE_DELAY_MS) || args.ttsRetryBaseDelayMs;
  }
  if (process.env.ENGLISH_TTS_RETRY_MAX_DELAY_MS) {
    args.ttsRetryMaxDelayMs = Number(process.env.ENGLISH_TTS_RETRY_MAX_DELAY_MS) || args.ttsRetryMaxDelayMs;
  }
  if (process.env.ENGLISH_TTS_RETRY_JITTER) {
    args.ttsRetryJitter = !/^false|0|no$/i.test(String(process.env.ENGLISH_TTS_RETRY_JITTER));
  }
  if (process.env.ENGLISH_TTS_PAUSE_ON_429 || process.env.ENGLISH_TTS_STOP_ON_QUOTA) {
    args.stopOnQuota = !/^false|0|no$/i.test(String(process.env.ENGLISH_TTS_PAUSE_ON_429 || process.env.ENGLISH_TTS_STOP_ON_QUOTA));
  }
  if (!args.explicit.storageFolder && process.env.ENGLISH_AUDIO_POLISH_STORAGE_FOLDER) {
    args.storageFolder = process.env.ENGLISH_AUDIO_POLISH_STORAGE_FOLDER.replace(/^\/+|\/+$/g, "") || args.storageFolder;
  }
  args.apiUrl = normalizeApiUrl(args.apiUrl);
}
...
function plaintextTtsInput(value) {
  return cleanText(decodeHtmlEntities(value))
    .replace(TTS_PAUSE_TOKEN_RE, (_match, ms) => (Number(ms) >= 300 ? "\n\n" : ", "))
    .replace(/<[^>]+>/g, " ")
    .replace(/[ \t]{2,}/g, " ")
    .replace(/[ \t]*\n[ \t]*/g, "\n")
    .replace(/\n{3,}/g, "\n\n")
...
function detectEnglishTerms(text) {
  ...
  for (const match of stripTtsPauseTokens(text).matchAll(ENGLISH_TOKEN_RE)) {
    const token = match[0];
    const initialism = /^(?:[A-Za-z]\.){2,}[A-Za-z]?\.?$/.test(token) || /^(?:A|P)\.?M\.?$/i.test(token);
    const risky = /\d/.test(token) || /[+#&/]/.test(token) || initialism;
    if (!risky) continue;
...
function normalizeEnglishTtsText(text, options = {}) {
  let output = cleanText(decodeHtmlEntities(text))
    .replace(/\.{3,}|…/g, "...")
    .replace(/[ \t]+([.?!,;:])/g, "$1")
    .replace(/([,;:?!])(?=\S)/g, "$1 ")
    .replace(/\n[ \t]+/g, "\n")
  ...
  const rules = [
    ...
    {
      name: "saint_abbreviation",
      regex: /\bSt\.\s+(?=[A-Z][A-Za-z'’-]+)/g,
      replace: "Saint ",
    },
    {
      name: "negative_number",
      regex: /(^|[\s([{\"'“‘])[-−]\s*([0-9]+(?:\.[0-9]+)?)/g,
      replace: (_match, prefix, number) => `${prefix}negative ${number}`,
    },
    {
      name: "numeric_range",
      regex: /([0-9]+(?:\.[0-9]+)?)\s*[-−]\s*([0-9]+(?:\.[0-9]+)?)/g,
      replace: (_match, left, right) => `${left} to ${right}`,
    },
    {
      name: "repeated_dash_pause",
      regex: /-{2,}|[—–]/g,
      replace: " [[break:180ms]] ",
    },
    {
  ...
  for (const rule of rules) {
    ...
    if ((rule.name.includes("english") || rule.name.includes("abbreviation")) && result.count) {
...
function isEnglishBook(book) {
  ...
  if (["en", "eng", "english", "en-us", "en-gb", "en-in"].includes(explicit)) return true;
  if (["bn", "ben", "bengali", "bn-in", "bn-bd"].includes(explicit)) return false;
  const surface = `${book.title || ""} ${book.author || ""} ${book.description || ""} ${book.short_description || ""}`;
  return LATIN_RE.test(surface) && !BENGALI_RE.test(surface);
}
...
async function requestJson(method, url, { headers = {}, body } = {}) {
  ...
  if (!response.ok) {
    const error = new Error(`${method} ${url} failed: HTTP ${response.status} ${text.slice(0, 300)}`);
...
class EarnalismAdminClient {
  ...
  async login(options = {}) {
    let token = options.forcePasswordLogin ? "" : String(process.env.EARNALISM_ADMIN_TOKEN || "").trim();
    if (!token) {
  ...
  async request(method, pathName, { body } = {}) {
    ...
    try {
      return await requestJson(method, url, { headers: this.headers, body });
    } catch (error) {
      const expired = Number(error.status || 0) === 401
        && /token expired|jwt expired|expired/i.test(`${error.message || ""} ${error.responseText || ""}`);
      if (!expired) throw error;
      await this.login({ forcePasswordLogin: true });
      return requestJson(method, url, { headers: this.headers, body });
    }
  }

  summaries() {
    return this.request("GET", "/admin/books/summary");
  }

  book(slug) {
    return this.request("GET", `/admin/books/${encodeURIComponent(slug)}`);
  }

  patchAudiobook(slug, payload) {
    return this.request("PATCH", `/admin/books/${encodeURIComponent(slug)}/audiobook`, {
      body: payload,
    });
  }
}
...
function bookNarrationText(book) {
  ...
  const text = normalizeEnglishPunctuation(parts.join("\n\n"));
  return { text, markers };
}
...
function renderLexicalTextFragment(rawText, lexicon, options = {}) {
...
function renderTextFragment(rawText, lexicon, options = {}) {
  ...
  const pieces = [];
  let cursor = 0;
  for (const match of raw.matchAll(TTS_PAUSE_TOKEN_RE)) {
    const start = match.index || 0;
    if (start > cursor) {
      pieces.push(renderLexicalTextFragment(raw.slice(cursor, start), lexicon, options));
    }
    pieces.push(breakTag(Math.min(Number(match[1]), (options.pauses || NATURAL_EXPRESSIVE_PAUSES).section)));
    cursor = start + match[0].length;
  }
  if (cursor < raw.length) {
    pieces.push(renderLexicalTextFragment(raw.slice(cursor), lexicon, options));
  }
  return pieces.join("");
}
...
function likelyChapterHeading(text) {
  ...
  return /^(chapter|book|part|volume|section|preface|prologue|epilogue|appendix|[ivxlcdm]+\.?|[0-9]+\.?)\b/i.test(cleaned);
}
...
function prosodyForProfile(profile, options = {}) {
  const baseRate = options.rate || process.env.ENGLISH_TTS_RATE || process.env.ENGLISH_TTS_SSML_RATE || DEFAULT_TTS_RATE;
  const basePitch = options.pitch || process.env.ENGLISH_TTS_PITCH || process.env.ENGLISH_TTS_SSML_PITCH || DEFAULT_TTS_PITCH;
  const profiles = {
    ...PROSODY_PROFILES,
...
function analyzeSsml(ssml, segments, options = {}) {
  ...
  const rate = options.rate || process.env.ENGLISH_TTS_RATE || process.env.ENGLISH_TTS_SSML_RATE || DEFAULT_TTS_RATE;
  const pitch = options.pitch || process.env.ENGLISH_TTS_PITCH || process.env.ENGLISH_TTS_SSML_PITCH || DEFAULT_TTS_PITCH;
  const ratePercent = Number(String(rate).match(/[-+]?\d+(?:\.\d+)?/)?.[0] || 0);
  const estimatedWordsPerMinute = Math.round(150 * (1 + ratePercent / 100));
  ...
  const pausesOver600ms = pauseDurations.filter((value) => value > NATURAL_EXPRESSIVE_PAUSES.section).length;
  ...
  return {
    ssmlProfileUsed: "azure-command-english-natural-expressive",
    rateUsed: rate,
    pitchUsed: pitch,
    pauseProfileUsed: options.pauseProfile || process.env.ENGLISH_TTS_PAUSE_PROFILE || DEFAULT_PAUSE_PROFILE,
    characterSpelledTerms: (String(ssml || "").match(/<say-as interpret-as="characters">/g) || []).length,
    averagePauseMs,
    maxPauseMs,
    pausesOver600ms,
...
function buildEnglishSsmlDocument(text, lexicon, options = {}) {
...
function buildEnglishSsml(text, lexicon, options = {}) {
  return buildEnglishSsmlDocument(text, lexicon, options).ssml;
}

function prepareNarration(book, lexicon, args = {}) {
  const { text: sourceText, markers } = bookNarrationText(book);
  const normalized = normalizeEnglishTtsText(sourceText, { lexicon });
  ...
  const plainWithPauses = text
    ...
    .replace(/,/g, ", ")
    .replace(/\n\s*\n/g, "\n\n");
  const ssmlDocument = buildEnglishSsmlDocument(text, lexicon, {
    rate: args.ttsRate,
    pitch: args.ttsPitch,
  ...
  return {
    ...
    ssmlStats: {
      ...
      lexiconTermsNormalized: normalized.stats.lexiconReplacementCount,
...
function providerProfile(provider, voice) {
  ...
  if (identity.includes("azure") || identity.includes("neerja") || identity.includes("prabhat")) {
    return { clarity: 9.0, pronunciation: 8.8, expression: 8.7 };
  }
...
function preparedQualityIssues(prepared) {
  ...
  const issues = [];
  if (stats.pausesOver600ms > 0) issues.push(`SSML has ${stats.pausesOver600ms} pauses above 600ms`);
...
function mixedLanguageStats(prepared) {
  ...
  const characterSpelledCount = Number(prepared.ssmlStats?.characterSpelledTerms || 0);
  const handledOccurrences = Math.min(occurrences, replacementCount + characterSpelledCount);
...
function scoreRegenerationReadiness({ provider, prepared }) {
  ...
  const pauseShapeOk = Number(stats.averagePauseMs || 0) > 0
    ...
    && Number(stats.pausesOver600ms || 0) === 0;
  const expressionOk = Number(stats.dialogueSegmentsDetected || 0) === 0
    || Number(stats.prosodyProfileCounts?.dialogue || 0) > 0;
  const pacingOk = Number(stats.estimatedWordsPerMinute || 0) >= 135
    && Number(stats.estimatedWordsPerMinute || 0) <= 170;
  ...
  if (stats.unsafeHyphenDashRemaining) issues.push("unsafe hyphen/dash/minus remains in final SSML text");
  if (!mixedOk) issues.push("English pronunciation lexicon coverage needs lexicon additions");
  ...
  return {
    ...
    lexicon_pronunciation_score: mixed,
    ...
    pacing_ok: pacingOk,
    lexicon_pronunciation_ok: mixedOk,
...
function scoreCurrentAudiobook(book, prepared, audit, threshold) {
  ...
  if (current.provider.includes("mms-tts")) issues.push("current provider is fast/open-source TTS; likely less expressive for premium English narration");
  if (prepared.englishTerms.length && !mixedStats.isIncidental && englishCoverage < 0.75) {
    issues.push("English terms need more pronunciation lexicon coverage");
  }
  ...
  if (wordsPerMinute && (wordsPerMinute < 95 || wordsPerMinute > 190)) {
    issues.push(`pacing outside English narration target: ${Math.round(wordsPerMinute)} wpm`);
  }
...
function scoreGeneratedBundle({ provider, prepared, durationMs, timestamps, supportsSsml, humanReviewed, threshold }) {
  ...
  const pausePenalty = Math.min(1.2, (ssmlStats.pausesOver600ms || 0) * 0.2 + Math.max(0, (ssmlStats.averagePauseMs || 0) - 260) / 220);
  ...
  const providerIdentity = `${provider?.name || ""} ${provider?.voice || ""} ${provider?.commandPreview || ""}`.toLowerCase();
  const edgeLike = providerIdentity.includes("edge") || providerIdentity.includes("neerja");
  const piperLike = providerIdentity.includes("piper");
  const plainProfile = edgeLike
    ? { base: 9.35, expression: 9.25, punctuation: 9.25, mixedBonus: 0.25 }
    : (piperLike
      ? { base: 8.85, expression: 8.55, punctuation: 8.65, mixedBonus: 0.05 }
      : { base: 8.8, expression: 8.4, punctuation: 8.5, mixedBonus: 0 });
  const base = providerConfigured ? (supportsSsml ? 9.45 : plainProfile.base) : 0;
  ...
  const expression = clampScore((supportsSsml ? 9.55 : plainProfile.expression) + (humanReviewed ? 0.25 : 0) - flatnessPenalty);
  const pacing = clampScore(wordsPerMinute ? 9.5 - Math.min(2.8, Math.abs(wordsPerMinute - 140) / 30) - pausePenalty : 6.0);
  const punctuation = clampScore((supportsSsml ? 9.6 : plainProfile.punctuation) + (monotonic ? 0.1 : -2) - pausePenalty);
  const mixed = clampScore(prepared.englishTerms.length ? 6.0 + englishCoverage * 3.5 + (supportsSsml ? 0.35 : plainProfile.mixedBonus) : 9.4 + plainProfile.mixedBonus);
...
function isTtsAuthError(error) {
  return Boolean(error?.isAuthError) || isAzureAuthTtsError(error);
}
...
function commandProvider({ name, command, supportsSsml, voice }) {
  return {
    name,
    configured: Boolean(command),
    supportsSsml: Boolean(supportsSsml),
    voice: voice || name,
    commandPreview: command,
    async synthesize({ inputText, ssml, outputPath, inputPath, index, slug } = {}) {
      if (!command) throw new Error(`${name} TTS command is not configured.`);
      const providerInput = this.supportsSsml ? ssml : inputText;
      await fsp.writeFile(inputPath, providerInput || "", "utf8");
      ...
      const { stdout } = await runShell(commandText, {
        ...
        EARNALISM_TTS_SSML: this.supportsSsml ? ssml : "",
        EARNALISM_TTS_TEXT: providerInput || "",
...
function createTtsProvider(name) {
  ...
  if (providerName === "none") {
    return {
      ...
      async synthesize() {
        throw new Error(
          "No English TTS provider configured. Set ENGLISH_TTS_PROVIDER=command and ENGLISH_TTS_COMMAND, or run dry-run audit only.",
        );
      },
    };
  }
  if (providerName === "azure") {
    const command = String(process.env.AZURE_ENGLISH_TTS_COMMAND || process.env.ENGLISH_TTS_COMMAND || "node scripts/audio/tts/azureEnglishTts.js").trim();
    return commandProvider({
      name: "azure",
      command,
      supportsSsml: true,
      voice: process.env.ENGLISH_TTS_VOICE_ID || process.env.AZURE_ENGLISH_VOICE || "en-IN-NeerjaNeural",
    });
  }
  if (providerName === "edge") {
    const voice = process.env.ENGLISH_TTS_VOICE_ID || process.env.EDGE_TTS_ENGLISH_VOICE || "en-IN-NeerjaNeural";
    const rate = process.env.ENGLISH_TTS_RATE || DEFAULT_TTS_RATE;
    const pitch = process.env.ENGLISH_TTS_PITCH || DEFAULT_TTS_PITCH;
    const command = String(process.env.EDGE_TTS_COMMAND || `edge-tts --voice ${shellQuote(voice)} --rate=${shellQuote(rate)} --pitch=${shellQuote(pitch)} --file "{input}" --write-media "{output}"`).trim();
    return commandProvider({
      name: "edge",
      command,
      supportsSsml: false,
  ...
  if (providerName === "piper") {
    const model = process.env.PIPER_ENGLISH_MODEL || "";
    const voice = process.env.ENGLISH_TTS_VOICE_ID || (model ? path.basename(model, path.extname(model)) : "piper");
    const command = String(process.env.PIPER_TTS_COMMAND || 'scripts/audio/tts/piperEnglishTts.sh "{input}" "{output}"').trim();
    return commandProvider({
      name: "piper",
      command,
      supportsSsml: false,
  ...
  if (providerName !== "command") {
    throw new Error(`Unsupported English TTS provider adapter: ${providerName}. Supported adapters: azure, edge, piper, command.`);
  }
  const command = String(process.env.ENGLISH_TTS_COMMAND || "").trim();
  if (!command) {
    return {
      name: "command",
      configured: false,
      supportsSsml: false,
      voice: process.env.ENGLISH_TTS_VOICE_ID || "",
      async synthesize() {
        throw new Error("ENGLISH_TTS_COMMAND is required when ENGLISH_TTS_PROVIDER=command.");
      },
    };
  }
  return commandProvider({
    name: "command",
    command,
    supportsSsml: /^true|1|yes$/i.test(String(process.env.ENGLISH_TTS_SUPPORTS_SSML || "")),
    voice: process.env.ENGLISH_TTS_VOICE_ID || "command",
  });
}

function runShell(command, extraEnv = {}) {
  const timeoutMs = Number(process.env.ENGLISH_TTS_COMMAND_TIMEOUT_MS || DEFAULT_TTS_COMMAND_TIMEOUT_MS);
...
async function synthesizeWithRetry(provider, payload, args) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      lastError = error;
      if (isAzureAuthTtsError(error)) {
        throw new TtsAuthError(
          "Azure subscription/key/region/resource is inactive or invalid. "
          + "Switch ENGLISH_TTS_PROVIDER to edge, piper, or command for free-provider polishing, "
          + "or fix AZURE_SPEECH_KEY and AZURE_SPEECH_REGION before using Azure again.",
        );
      }
      if (isTtsQuotaError(error)) {
        ...
        if (!shouldRetry) {
          const quota = new TtsQuotaError(error.message || "Azure English TTS quota exceeded");
...
function ttsProviderCacheIdentity(provider) {
  return [
    provider?.name || "none",
    provider?.supportsSsml ? "ssml" : "plaintext",
    provider?.voice || "",
  ].join("|");
}
...
async function generateBundle({ book, prepared, provider, args, progress, progressPath }) {
  ...
  const bundleDir = path.join(args.jobDir, "bundles", "en", slug);
  const tempDir = path.join(bundleDir, "_work");
  const chunks = chunkText(prepared.normalizedText, args.chunkChars);
  const wavPaths = [];
  const timestamps = [];
  const providerIdentity = ttsProviderCacheIdentity(provider);
  let offsetMs = 0;
  ...
  try {
    for (let index = 0; index < chunks.length; index += 1) {
      ...
      const ssml = buildEnglishSsml(chunk, args.lexiconMap, {
        rate: args.ttsRate,
        pitch: args.ttsPitch,
        pauseProfile: args.pauseProfile,
      });
      const providerInputText = provider.supportsSsml ? chunk : plaintextTtsInput(chunk);
      const providerInput = provider.supportsSsml ? ssml : providerInputText;
      const chunkHash = sha256Short(`${providerIdentity}\n${providerInput}`);
      const key = chunkProgressKey(index);
      const inputPath = path.join(tempDir, `chunk_${key}_${chunkHash}.${provider.supportsSsml ? "ssml" : "txt"}`);
      const rawOutput = path.join(tempDir, `chunk_${key}_${chunkHash}.${process.env.ENGLISH_TTS_OUTPUT_FORMAT || "mp3"}`);
      const wavPath = path.join(tempDir, `chunk_${key}_${chunkHash}.wav`);
      await fsp.writeFile(inputPath, providerInput, "utf8");
      const savedChunk = progress?.chunks?.[slug]?.[key] || {};
      const savedProviderIdentity = savedChunk.provider_identity
        || (savedChunk.provider || savedChunk.voice || savedChunk.supports_ssml !== undefined
          ? [
            savedChunk.provider || "",
            savedChunk.supports_ssml ? "ssml" : "plaintext",
            savedChunk.voice || "",
          ].join("|")
          : "");
      const providerChanged = Boolean(savedChunk.chunk_hash && savedProviderIdentity && savedProviderIdentity !== providerIdentity);
      const rawExists = await fileExistsWithBytes(rawOutput);
      const wavExists = await fileExistsWithBytes(wavPath);
      const mixedRawOutput = String(savedChunk.output_path || "");
      const mixedWavPath = String(savedChunk.wav_path || "");
      const mixedProviderCacheAllowed = providerChanged
        && args.reuseMixedProvider
        && !args.forceRegenerate
        && await fileExistsWithBytes(mixedRawOutput)
        && await fileExistsWithBytes(mixedWavPath);
      const activeRawOutput = mixedProviderCacheAllowed ? mixedRawOutput : rawOutput;
      const activeWavPath = mixedProviderCacheAllowed ? mixedWavPath : wavPath;
      const cacheMatches = savedChunk.chunk_hash === chunkHash
        && !args.forceRegenerate
        && rawExists
        && wavExists;
      const hasRawOutput = cacheMatches || mixedProviderCacheAllowed || (!args.forceRegenerate && rawExists && !providerChanged);
      const hasWavOutput = cacheMatches || mixedProviderCacheAllowed || (!args.forceRegenerate && wavExists && !providerChanged);
      const chunkMetadata = {
        book_slug: slug,
        ...
        supports_ssml: provider.supportsSsml,
        plaintext_input_used: !provider.supportsSsml,
        provider_changed: providerChanged,
        mixed_provider_reused: mixedProviderCacheAllowed,
        regenerated_due_to_provider_change: providerChanged && !mixedProviderCacheAllowed && !hasRawOutput,
      };
      if (cacheMatches) {
        await saveChunkProgress({
          ...
          update: {
            ...
            output_path: activeRawOutput,
            wav_path: activeWavPath,
      ...
      if (!hasRawOutput) {
        ...
        if (ttsResult?.ttsMeta) {
          progress.tts = {
            ...
            plaintextInputUsed: !provider.supportsSsml,
      ...
      const chunkDuration = await durationMs(activeWavPath);
      timestamps.push(...syntheticTimestamps(providerInputText, offsetMs, chunkDuration));
      wavPaths.push(activeWavPath);
      offsetMs += chunkDuration;
      const verb = hasRawOutput && hasWavOutput
        ? (mixedProviderCacheAllowed ? "reused mixed-provider" : "reused")
        : "synthesized";
      process.stdout.write(`${slug}: ${verb} ${index + 1}/${chunks.length} chunks\n`);
    ...
    if (/^false|0|no$/i.test(String(process.env.ENGLISH_TTS_KEEP_CHUNKS || "true"))) {
...
async function uploadBundle({ slug, bundle, provider, args, generatedScore }) {
  ...
  for (const [assetKind, filePath, fileName, duration] of uploadPlan) {
    const publicId = `${args.storageFolder}/en/${slug}/${fileName}`;
...
function verboseDebugPayload({ slug, prepared, args, provider }) {
  return {
    ...
    provider: provider.name,
    supportsSsml: provider.supportsSsml,
    ttsCommand: redactSecrets(provider.commandPreview || process.env.ENGLISH_TTS_COMMAND || ""),
    ...
    pauseDistribution: {
      ...
      pausesOver600ms: prepared.ssmlStats.pausesOver600ms,
...
async function processBook({ summary, client, args, provider, progress, progressPath }) {
  ...
  if (!isEnglishBook(book)) {
    ...
    row.detected_issues = ["book does not look English from metadata/title/description"];
    return row;
  }
  ...
  Object.assign(row, {
    ...
    lexiconTermsNormalized: prepared.ssmlStats.lexiconTermsNormalized,
    roboticFlatnessRisk: prepared.ssmlStats.roboticFlatnessRisk,
  });
  ...
  let cachedBundle = args.forceRegenerate ? null : await loadCachedBundle(slug, args);
  if (cachedBundle && !args.reuseMixedProvider) {
    const cachedProvider = String(cachedBundle.meta?.provider_used || "");
    const cachedVoice = String(cachedBundle.meta?.voice || "");
    const providerChanged = cachedProvider && cachedProvider !== provider.name;
    const voiceChanged = cachedVoice && cachedVoice !== provider.voice;
    if (providerChanged || voiceChanged) {
      process.stdout.write(`${slug}: ignoring cached bundle from ${cachedProvider || "unknown"}:${cachedVoice || "unknown"} because current provider is ${provider.name}:${provider.voice || "default"}\n`);
      cachedBundle = null;
    }
  }
  if (cachedBundle) {
...
function selectSummaries(summaries, args) {
  ...
  if (requested.length) {
    ...
    const englishAudiobooks = summaries.filter((item) => isEnglishBook(item) && hasAudiobook(item));
    if (args.sample > 0) selected = shuffle(englishAudiobooks).slice(0, args.sample);
    else if (args.allEnglish) selected = englishAudiobooks;
    else throw new Error("Choose --sample N, --slug book-slug, --manifest PATH, or --all-english.");
  }
...
async function buildEnglishAudiobookQueue({ client, summaries, args }) {
  const selectionArgs = {
    ...args,
    allEnglish: args.allEnglish || (!args.slug.length && !args.manifest && !args.sample),
    sample: args.sample,
  };
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    const book = await client.book(slug);
    if (!isEnglishBook(book)) continue;
    const current = currentAudioInfo(book);
    const prepared = prepareNarration(book, args.lexiconMap, args);
  ...
  return {
    generated_at: new Date().toISOString(),
    queue_type: "english-audiobook-regeneration",
    sort: args.queueSort,
...
function chunkProgressSummary(progress, failed) {
  ...
  const providerChanged = rows.some((row) => row.provider_changed);
  const chunksRegeneratedDueToProviderChange = rows.filter((row) => row.regenerated_due_to_provider_change).length;
  const plaintextInputUsed = rows.some((row) => row.plaintext_input_used) || Boolean(progress.tts?.plaintextInputUsed);
  ...
  return {
    ...
    providerChanged,
    chunksRegeneratedDueToProviderChange,
    plaintextInputUsed,
  };
}
...
async function writeReport(args, progress, selected) {
  ...
  const report = {
    ...
    ttsSupportsSsml: Boolean(progress.tts?.supportsSsml),
    voice: progress.tts?.voice || process.env.ENGLISH_TTS_VOICE_ID || process.env.AZURE_ENGLISH_VOICE || "",
    region: progress.tts?.region || process.env.AZURE_SPEECH_REGION || "",
    fallbackRegionUsed: chunkSummary.fallbackRegionUsed || "",
    azureAuthBlocked: failed.some((row) => row.status === "BLOCKED_AUTH"),
    providerChanged: chunkSummary.providerChanged,
    chunksRegeneratedDueToProviderChange: chunkSummary.chunksRegeneratedDueToProviderChange,
    ...
    plaintextInputUsed: chunkSummary.plaintextInputUsed,
    ...
    summary: {
      ...
      auth_blocked: failed.filter((row) => row.status === "BLOCKED_AUTH").length,
    },
    books: rows,
    failed,
  };
...
function isAzureAuthTtsError(error) {
  const message = String(error?.message || error || "");
  return /Azure English TTS failed:\s*HTTP\s*401/i.test(message);
}

async function selectedRequiresTtsSynthesis(selected, args, progress) {
  if (args.dryRun || !selected.length) return false;
  ...
  for (const summary of selected) {
    ...
    if (args.resume && completedRow && !shouldReprocessManualReview) continue;
    const cachedBundle = await loadCachedBundle(slug, args);
    if (!cachedBundle) return true;
  }
  return false;
}

async function runTtsPreflightIfNeeded({ selected, args, provider, progress }) {
  if (!await selectedRequiresTtsSynthesis(selected, args, progress)) return;
  if (!provider?.configured) return;

  const preflightDir = path.join(args.jobDir, "_preflight");
  await ensureDir(preflightDir);
  const inputText = "Hello from Earnalism.";
  const inputPath = path.join(preflightDir, `english-tts-preflight.${provider.supportsSsml ? "ssml" : "txt"}`);
  const outputPath = path.join(preflightDir, "english-tts-preflight.mp3");
  const ssml = buildEnglishSsml(inputText, args.lexiconMap || {}, {
    rate: args.ttsRate,
    pitch: args.ttsPitch,
    pauseProfile: args.pauseProfile,
  });
  ...
  try {
    await provider.synthesize({
      ...
      outputPath,
      index: -1,
      slug: "english-tts-preflight",
    });
    process.stdout.write("English TTS preflight: ok\n");
  } catch (error) {
    if (isTtsAuthError(error)) {
      throw new TtsAuthError(
        "English TTS preflight failed before audiobook generation: Azure returned HTTP 401. "
        + "Azure subscription/key/region/resource is inactive or invalid. "
        + "Set ENGLISH_TTS_PROVIDER=edge, ENGLISH_TTS_PROVIDER=piper, or ENGLISH_TTS_PROVIDER=command for free-provider polishing, "
        + "or fix AZURE_SPEECH_KEY and AZURE_SPEECH_REGION before using Azure again. Existing completed chunks are preserved."
      );
    }
    throw error;
  }
...
async function runNormalizationSelfTest(args) {
  ...
  const normalize = (value) => normalizeEnglishTtsText(value, { lexicon }).text;
  const ssmlFor = (value) => buildEnglishSsmlDocument(normalize(value), lexicon, {
    rate: args.ttsRate,
    pitch: args.ttsPitch,
    pauseProfile: args.pauseProfile,
  }).ssml;
  ...
  const cases = [
    ["self-help", "self help"],
    ["AI audiobook", "A I audio book"],
    ["1901-1905", "1901 to 1905"],
    ["10-12 minutes", "10 to 12 minutes"],
    ["-5 degrees", "negative 5 degrees"],
  ...
  const dashSsml = ssmlFor("She paused — then spoke.");
  assertSelfTest(dashSsml.includes('<break time="180ms"/>'), "em dash should become 180ms break", { dashSsml });
  const defaultSsml = ssmlFor("She spoke softly.");
  assertSelfTest(defaultSsml.includes('rate="+4%"'), "default narration should use +4% rate", { defaultSsml });
  assertSelfTest(defaultSsml.includes('pitch="+1Hz"'), "default narration should use +1Hz pitch", { defaultSsml });
  assertSelfTest(isAzureAuthTtsError(new Error("Azure English TTS failed: HTTP 401")), "Azure English HTTP 401 auth errors should be detected");
  ...
  assertSelfTest(
    isTtsQuotaError(new Error("Azure English TTS failed: HTTP 429 Quota Exceeded")),
    "Azure HTTP 429 quota errors should be detected",
  );
  assertSelfTest(
    retryAfterMs(new Error("Azure English TTS failed: HTTP 429 Quota Exceeded; retry_after=60")) === 60000,
    "Azure Retry-After seconds should be converted to milliseconds",
  );
  const plaintext = plaintextTtsInput('Hello <break time="180ms"/> world [[break:380ms]] Earnalism.');
  assertSelfTest(!/[<>]|\[\[break:/i.test(plaintext), "non-SSML providers should receive plaintext without SSML/break tags", { plaintext });
  assertSelfTest(plaintext.includes("Hello") && plaintext.includes("world"), "plaintext conversion should preserve words", { plaintext });
  assertSelfTest(
    chunkText("One. Two. Three. Four. Five. ".repeat(1200), DEFAULT_CHUNK_CHARS).every((chunk) => chunk.length <= 4000),
...
async function main() {
  ...
  if (args.exportQueue) {
    const queue = await buildEnglishAudiobookQueue({ client, summaries, args });
    await writeJson(args.exportQueue, queue);
    process.stdout.write(`English audiobook queue written: ${args.exportQueue}\n`);
  ...
  progress.tts = {
    ...
    supportsSsml: provider.supportsSsml,
    plaintextInputUsed: !provider.supportsSsml,
    voice: provider.voice || progress.tts?.voice || "",
    region: provider.name === "azure" ? (process.env.AZURE_SPEECH_REGION || progress.tts?.region || "") : (progress.tts?.region || ""),
  };
  await saveProgress(progressPath, progress);

  process.stdout.write(`English audiobook polish job: ${args.jobId}\n`);
  ...
  try {
    await runTtsPreflightIfNeeded({ selected, args, provider, progress });
  } catch (error) {
    if (!isTtsAuthError(error)) throw error;
    const first = selected[0] || {};
    const slug = normalizeSlug(first.slug || "english-tts-preflight");
    ...
    process.stderr.write(`BLOCKED_AUTH ${slug}: ${error.message}\n`);
    process.stdout.write("\nEnglish audiobook polish summary\n");
    process.stdout.write("================================\n");
  ...
  await runPool(selected, args.concurrency, async (summary, index) => {
    ...
    try {
      ...
      const authError = isTtsAuthError(error);
      progress.failed = progress.failed || {};
      progress.failed[slug] = {
        book_slug: slug,
        title: summary.title || "",
        status: authError ? "BLOCKED_AUTH" : (quotaError ? "PAUSED_QUOTA" : "BLOCKED"),
        detail: error.stack || error.message || String(error),
      ...
      process.stderr.write(`${authError ? "BLOCKED_AUTH" : (quotaError ? "PAUSED_QUOTA" : "BLOCKED")} ${slug}: ${error.message}\n`);
      if (authError) {
        stopRequested = true;
        stopReason = "Azure English TTS authentication is blocked. Use ENGLISH_TTS_PROVIDER=edge, piper, or command for free-provider polishing, or repair Azure credentials before resuming.";
      } else if (quotaError && args.stopOnQuota) {
        stopRequested = true;
